...
| IF implementing a helper that coordinates agents with its own run status, cursor, retry, or resume protocol, THEN put that coordination in Norn workflows and worker sessions, leaving mechanical or domain work in helpers. ELSE keep a bounded helper call local. | The workflow assesses sources with Norn workers, retains accepted results, and transitions to delivery; its CLI shim only starts or continues the run. | Register a workflow that shells out to an independent Pi coordinator with its own locks, cursors, and recovery protocol. |
...
| IF a command or worker prevents the requested workflow outcome, THEN expose the failure or interruption through Norn and retain a boundary from which that work can continue. ELSE complete only the verified workflow outcome. | Delivery is unavailable: accepted assessments remain saved and the Norn run identifies the pending delivery phase. | Convert the delivery error to success and complete a wrapper while a separate HANDOFF file contains the operation's actual pending state. |
