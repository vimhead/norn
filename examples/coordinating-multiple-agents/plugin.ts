...
const notesSchema = Type.Refine(
	Type.Array(noteSchema, { minItems: 2, maxItems: 12 }),
...
const workerReportSchema = Type.Object(
	{
		status: Type.Enum(["acknowledged", "idle", "blocked"]),
	...
	async execute({ args, paths }) {
		...
		try {
			for (const note of args.notes)
				await queue.enqueue({ ...note, signal: undefined });
...
export const work = scope.workflow({
	...
	args: Type.Object(
		{
			...
			round: Type.Integer({ minimum: 0, maximum: 12 }),
		},
		{ additionalProperties: false },
	),
	async execute({ args, paths, agents, run }): Promise<WorkflowResult> {
		...
		try {
			...
			if (before.items.length !== args.notes.length)
				return run.fail({
					summary: "Queue inventory differs from the supplied notes.",
				});
			if (before.acknowledged === args.notes.length)
				return verify({ notes: args.notes });
			if (before.leased > 0 || args.round >= args.notes.length)
				return run.fail({
					summary:
						"Unfinished claims or exhausted rounds; inspect queue and agent logs before recovery.",
				});
			const reports = await processRound({
				agents,
				cwd: paths.workspace,
				queue,
				round: args.round,
			...
			await writeFile(
				join(paths.workspace, `rounds/${args.round}.json`),
			...
			if (
				...
				return run.fail({
					summary:
						"The agent round did not finish its claims; inspect the saved reports and queue before recovery.",
				});
			}
...
export const verify = scope.workflow({
	...
	async execute({ args, paths, run }) {
		...
		try {
			...
			if (
				snapshot.items.length !== args.notes.length ||
				snapshot.acknowledged !== args.notes.length
			)
				return run.fail({ summary: "Some notes have no persisted result." });
			const results = args.notes.map((note) => {
				...
				if (
					...
					item.text !== note.text ||
					!note.text.includes(item.result.quote)
				)
					throw new Error(`Unverified result or source quotation: ${note.id}`);
				return {
					id: note.id,
					source: note.text,
					...item.result,
			...
			await writeFile(
				join(paths.workspace, summariesPath),
				JSON.stringify(results, null, 2),
			);
			return run.complete({
				summary:
					"All queue results persisted; schemas and source quotations checked.",
				data: { summariesPath, processed: results.length },
			});
		} finally {
			queue.close();
		}
...
function openQueue(input: {
	readonly workspace: string;
	readonly create: boolean;
}) {
	return WorkQueue.open({
		path: join(input.workspace, "queue.sqlite"),
		create: input.create,
...
async function processRound(input: {
...
then report the tool-confirmed outcome.`,
	...
	if (errors.length)
		throw new AggregateError(
			errors,
			"Queue agents failed; inspect their retained agent logs",
		);
	return reports;
}
