...
export const analyze = scope.workflow({
	...
	async execute({ args, paths, agents, run }) {
		const savedDraft = Value.Parse(
			savedDraftSchema,
			JSON.parse(await readFile(join(paths.workspace, args.draftPath), "utf8")),
		);
		const invalidQuotations = savedDraft.draft.quotations.filter(
			(quotation) => !savedDraft.source.includes(quotation),
		);
		...
		await writeFile(
			join(paths.workspace, analysisPath),
