...
The workflow seeds source state. Its worker receives only read access to the source and write access to the copy through [`StateAdapter`](../../docs/resources.md), passed in `resourceAdapters`. It requests no filesystem task tools. After the worker session closes, a transition checkpoints the values; the next workflow checks exact equality and writes `current/artifacts/copy.txt`. Missing or different output fails instead of trusting the worker's response.
