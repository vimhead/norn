...
export type SharedStateField<Schema extends TSchema = TSchema> = {
	readonly id: string;
	readonly schema: Schema;
};
export type SharedStateAccess = Pick<
	SharedState,
	"get" | "getOptional" | "set"
>;
...
export class SharedState {
	...
	async get<Schema extends TSchema>(
		field: SharedStateField<Schema>,
	): Promise<Static<Schema>> {
		const value = await this.getOptional(field);
		if (value === undefined)
			throw new Error(`Missing shared state: ${field.id}`);
		return value;
	}

	async getOptional<Schema extends TSchema>(
		field: SharedStateField<Schema>,
		...
		return row === undefined
			? undefined
			: copyValue(field.schema, JSON.parse(String(row.value)));
	}

	async set<Schema extends TSchema>(
		field: SharedStateField<Schema>,
		value: NoInfer<Static<Schema>>,
	): Promise<void> {
		...
		this.database
			...
			.run(field.id, JSON.stringify(checked));
	}
...
function copyValue<Schema extends TSchema>(
	schema: Schema,
	value: unknown,
): Static<Schema> {
