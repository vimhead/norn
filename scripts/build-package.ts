...
async function emitPackageDeclarations(): Promise<void> {
	...
	try {
		...
		const result = spawnSync(
			process.execPath,
			[compilerPath, "--project", configPath],
			{ cwd: workspaceRoot, stdio: "inherit" },
		);
		if (result.error) throw result.error;
		if (result.status !== 0)
			throw new Error(
				`Declaration emission failed: ${packageRoot} (exit ${result.status}, signal ${result.signal})`,
			);
		await cp(
			join(declarationRoot, relative(workspaceRoot, sourceRoot)),
			outputRoot,
			{ recursive: true },
		);
	} finally {
