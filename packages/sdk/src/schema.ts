...
export function inspectSchema(schema: TSchema): NornJsonSchema {
	...
	if (!isPlainObject(inspected))
		throw new Error("Norn inspection requires an object schema");
...
function assertWorkflowReferenceAnnotations(schema: unknown): void {
	...
	if (Object.hasOwn(schema, "x-norn-workflow-ref")) {
		...
		if (
			...
			!Object.hasOwn(annotation, "contributedArgsSchema")
		)
			throw new Error("Invalid workflow reference annotation");
		Value.Assert(
			Meta["https://json-schema.org/draft/2020-12/schema"],
			annotation.contributedArgsSchema,
		);
	...
	for (const keyword of [
		...
		if (isPlainObject(schemas))
			for (const child of Object.values(schemas))
				assertWorkflowReferenceAnnotations(child);
	}
	for (const keyword of ["allOf", "anyOf", "oneOf", "prefixItems"]) {
		const schemas = schema[keyword];
		if (Array.isArray(schemas))
			for (const child of schemas) assertWorkflowReferenceAnnotations(child);
	}
	for (const keyword of [
...
export function assertWorkflowMetadata(
	...
	if (!isPlainObject(entrypoint))
		throw new Error(
			`Workflow entrypoint must be false or an object with instructions: ${workflow.id}`,
		);
	if (
		typeof entrypoint.instructions !== "string" ||
		entrypoint.instructions.trim().length === 0
	) {
		throw new Error(
			`Workflow entrypoint.instructions must be a nonempty string: ${workflow.id}`,
		);
	}
}
...
export function schemaType(schema: TSchema): string | undefined {
	const unwrapped = unwrapSchema(schema);
	return "type" in unwrapped && typeof unwrapped.type === "string"
		? unwrapped.type
		: undefined;
}
