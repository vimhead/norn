...
import {
	...
	type NornDocumentationBundle,
} from "./internal/documentation-bundle.ts";
...
export function resolveDocumentationCacheRoot(input: {
	...
	if (input.platform === "win32")
		return join(
			input.environment.LOCALAPPDATA || join(input.home, "AppData", "Local"),
			"norn",
			"docs",
		);
	if (input.platform === "darwin")
		return join(input.home, "Library", "Caches", "norn", "docs");
	const xdgCache = input.environment.XDG_CACHE_HOME;
	return join(
		xdgCache?.startsWith("/") ? xdgCache : join(input.home, ".cache"),
		"norn",
		"docs",
	);
}
...
export async function resolveNornDocumentation(input: {
	...
	if (input.source.kind === "local") {
		...
		if (version !== input.build.version)
			throw new Error(
				"Local documentation version does not match the runtime; rebuild or reinstall this CLI package",
			);
		...
		if (version !== input.build.version)
			throw new Error(
				"Embedded documentation version does not match the binary build",
			);
		...
		const cacheKey = createHash("sha256")
			...
			.digest("hex");
		root = await new NornDocumentationCache({
...
async function validateLocalDocumentation(root: string): Promise<void> {
	for (const [key, path] of Object.entries(DOCUMENTATION_PATHS)) {
		...
		const isExpectedType =
			key === "docs" || key === "examples" ? stat.isDirectory() : stat.isFile();
		if (!isExpectedType)
			throw new Error(`Invalid local documentation path: ${join(root, path)}`);
	}
}
...
class NornDocumentationCache {
	...
	constructor(
		private readonly input: {
			...
			readonly bundle: NornDocumentationBundle;
		},
	) {
		this.root = join(input.cacheRoot, `v1-${input.cacheKey}`);
		this.files = new Map(
			input.bundle.files.map((file) => [file.path, file.content]),
		);
	}
	...
	async materialize(): Promise<string> {
		...
		if (!cacheStat.isDirectory() || cacheStat.isSymbolicLink())
			throw new Error(
				`Documentation cache root must be a real directory: ${this.input.cacheRoot}`,
			);
		...
		try {
			for (const file of this.input.bundle.files) {
				...
				await mkdir(join(staging, ...parts.slice(0, -1)), {
					recursive: true,
					mode: 0o700,
				});
				await writeFile(join(staging, ...parts), file.content, {
					encoding: "utf8",
					flag: "wx",
					mode: 0o600,
				});
			...
			if (!(await this.validateExistingCache()))
				throw new Error(
					`Documentation cache disappeared after extraction: ${this.root}`,
				);
			return this.root;
		} finally {
	...
	private async validateDirectory(input: {
		...
		for (const entry of await readdir(input.directory, {
			...
			if (entry.isDirectory()) {
				...
				await this.validateDirectory({
					...
					prefix: path,
					seenFiles: input.seenFiles,
				});
			} else if (entry.isFile() && this.files.has(path)) {
				if (
					!(await readFile(absolutePath)).equals(
						Buffer.from(this.files.get(path)!, "utf8"),
					)
				)
					throw this.invalidCacheError();
	...
	private invalidCacheError(): Error {
		return new Error(
			`Documentation cache is incomplete or modified: ${this.root}. Remove this cache entry and retry; copy examples elsewhere before editing them.`,
		);
	}
}
