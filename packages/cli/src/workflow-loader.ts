...
import {
	...
	type NornDispose,
	type NornWorkflowDiagnostic,
	type NornProjectInfo,
	type NornProjectInspection,
	type NornProjectConfigurationInfo,
	type NornWorkflowCatalogInfo,
	type NornWorkflowInspection,
...
} from "./internal/workflow-registry.ts";
...
type NornProjectConfigFile = Omit<NornConfigFile, "config"> & {
...
export async function discoverNornProject(
	cwd: string,
): Promise<NornProjectInspection & NornWorkflowCatalogInfo> {
	const { project, diagnostics } = await collectNornProject(cwd);
	return {
		project: buildProjectInfo(project),
		workflows: project.registry.list(),
		isComplete: diagnostics.length === 0,
		diagnostics,
	};
}
export async function inspectNornWorkflow(input: {
	readonly cwd: string;
	readonly workflowId: string;
}): Promise<NornWorkflowInspection> {
	const { project, diagnostics } = await collectNornProject(input.cwd);
	const summary = project.registry
		.list()
		.find((workflow) => workflow.id === input.workflowId);
	if (!summary) {
	...
	try {
		return {
			workflow: project.registry.inspect(input.workflowId) ?? null,
			isComplete: diagnostics.length === 0,
			diagnostics,
		};
	} catch (error) {
...
export async function findNornProject(cwd: string): Promise<NornProject> {
	...
	const projectFile = await readConfigFile({
		path: projectPath,
		isProject: true,
	});
	...
	return {
		cwd: resolve(cwd),
		projectPath,
		projectRoot: projectFile.root,
		configPath: projectPath,
		configRoot: projectFile.root,
...
async function readConfigFile(input: {
...
}): Promise<NornProjectConfigFile> {
	const raw: unknown = JSON.parse(await readFile(input.path, "utf8"));
	if (isPlainObject(raw) && Object.hasOwn(raw, "plugins"))
		throw new Error(
			`Replace plugins with workflows in ${input.path}; registration modules must export workflow arrays`,
		);
	...
	return {
		path: input.path,
		root: dirname(input.path),
		config: { ...config, version: 1 },
	};
}
async function collectNornProject(cwd: string): Promise<{
	readonly project: NornLoadedProject;
	readonly diagnostics: NornWorkflowDiagnostic[];
}> {
			...
			try {
				...
				errors.push(
					createDiagnostic({
						...
						scopeId: workflow.scope?.id ?? null,
						stage,
						error,
					}),
				);
	...
	return {
		project: {
			...project,
			definitions,
			modules: loaded.map((module) => module.source),
			configurations: registry.configurationInfos(),
			registry,
			workflows: registry.launchableEntries(),
		},
		diagnostics,
	};
}
function collectModuleConflicts(
	modules: readonly ImportedWorkflowModule[],
): Map<ImportedWorkflowModule, NornWorkflowDiagnostic[]> {
	...
	for (const module of modules)
		for (const workflow of module.definitions)
			byId.set(workflow.id, [...(byId.get(workflow.id) ?? []), module]);
	for (const [id, owners] of byId) {
		if (owners.length < 2) continue;
		for (const module of new Set(owners))
			conflicts.set(module, [
				...(conflicts.get(module) ?? []),
				createDiagnostic({
					source: module.source,
					workflowId: id,
					scopeId: null,
					stage: "duplicate",
					error: new Error(`Workflow already registered: ${id}`),
				}),
			]);
	}
	for (let index = 0; index < modules.length; index++) {
		for (let other = index; other < modules.length; other++) {
			...
			try {
				assertCompatibleConfigurationOwners(
					candidates.flatMap((module) => module.definitions),
				);
			} catch (error) {
...
async function importWorkflowModules(project: NornProject): Promise<{
	readonly modules: ImportedWorkflowModule[];
	readonly diagnostics: NornWorkflowDiagnostic[];
}> {
	const modules: ImportedWorkflowModule[] = [];
	const diagnostics: NornWorkflowDiagnostic[] = [];
	const jiti = createJiti(pathToFileURL(project.configPath).href, {
		moduleCache: false,
		tryNative: false,
		virtualModules: nornWorkflowVirtualModules(),
	});
	const cache: ModuleCache = Object.create(null);
	for (const file of project.configFiles)
		for (const path of file.config.workflows) {
			const source = {
				configPath: file.path,
				path: resolveConfigPath(file.root, path),
			};
			...
			try {
				...
				const module = (
					cached?.loaded
						? cached.exports
						: await jiti.evalModule(await readFile(source.path, "utf8"), {
				...
				if (
					...
					!Array.from(module.default).every(isWorkflowDeclaration)
				)
					throw new Error(
						`Workflow module must default-export an array of complete workflow definitions: ${source.path}`,
					);
				modules.push({ definitions: module.default, source });
			} catch (error) {
				for (const [path, module] of Object.entries(cache))
					if (!module.loaded) delete cache[path];
				diagnostics.push(
					createDiagnostic({
						source,
						workflowId: null,
...
function createDiagnostic(input: {
	...
	readonly stage: NornWorkflowDiagnostic["stage"];
	readonly error: unknown;
}): NornWorkflowDiagnostic {
	return {
		configPath: input.source.configPath,
		modulePath: input.source.path,
...
function mergeProjectConfig(
	...
	for (const file of reusable)
		config = mergeConfig({
			base: config,
			override: file.config.config,
			path: ["config"],
			rejectConflicts: true,
		});
	return mergeConfig({
		base: config,
		override: project?.config.config ?? {},
		path: ["config"],
	...
	for (const [key, value] of Object.entries(input.override)) {
		...
		if (isPlainObject(previous) && isPlainObject(value))
			result[key] = mergeConfig({
				...
				override: value,
				path,
				rejectConflicts: input.rejectConflicts,
			});
		else if (
			input.rejectConflicts &&
...
async function expandIncludePath(
	root: string,
	path: string,
): Promise<string[]> {
	const segments = path
	...
	return segments.includes("*")
		? expandGlobSegments(isAbsolute(path) ? sep : root, segments)
...
async function expandGlobSegments(
	...
	if (segment !== "*")
		return expandGlobSegments(join(root, segment), remaining);
	...
	return (
		await Promise.all(
			entries
				...
				.map((entry) => expandGlobSegments(join(root, entry.name), remaining)),
		)
	).flat();
}
