...
export class NornFileCoordinator {
	...
	constructor(
		...
		if (!Number.isSafeInteger(input.waitTimeoutMs) || input.waitTimeoutMs <= 0)
			throw new Error("Lock wait timeout must be a positive integer");
		this.lockRoot = resolve(input.lockRoot);
	}

	async withExclusiveLock<T>(
		path: string,
		operation: (lockedPath: string) => Promise<T>,
	): Promise<T> {
		...
		return this.withCleanup({
			operation: async () => {
				await writeFile(
					join(stagingPath, marker),
					JSON.stringify({ token, pid: process.pid, host: hostname(), target }),
					{ mode: 0o600 },
				);
	...
	async readText(path: string): Promise<string> {
		return this.withExclusiveLock(path, (lockedPath) =>
	...
	async writeText(path: string, content: string): Promise<void> {
		...
		await this.withExclusiveLock(path, (lockedPath) =>
			writeTextAtomically(lockedPath, content),
		);
	}

	private async withCleanup<T>(input: {
		...
		try {
			...
			try {
				...
				throw new AggregateError(
					[error, cleanupError],
					"File operation and lock cleanup failed",
				);
			}
			throw error;
		}
	...
	private async resolveTarget(path: string): Promise<string> {
		try {
			...
			if (entry?.isSymbolicLink())
				throw new Error(`Cannot lock a dangling symbolic link: ${path}`);
	...
	private async acquire(input: {
		...
		while (true) {
			...
			if (Date.now() >= deadline)
				throw new Error(`Timed out waiting for file lock: ${input.target}`);
			await delay(10);
		}
	}

	private async reclaimDeadOwner(input: {
		readonly lockPath: string;
		readonly target: string;
	}): Promise<void> {
		try {
			...
			if (entries.length !== 1)
				throw new Error(`Invalid file lock ownership: ${input.lockPath}`);
			const marker = entries[0];
			const owner = Value.Parse(
				ownerSchema,
				JSON.parse(await readFile(join(input.lockPath, marker), "utf8")),
			);
			if (
...
export function createRunFileCoordinator(runRoot: string): NornFileCoordinator {
	return new NornFileCoordinator({
		lockRoot: join(runRoot, "locks"),
