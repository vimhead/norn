...
export class NornRunLogger {
	...
	static async load(
		manifestPath: string,
		files: NornFileCoordinator,
	): Promise<NornRunLogger> {
	...
	async record(event: {
		...
		await this.input.files.withExclusiveLock(
			this.input.manifestPath,
			async (path) => {
				...
				if (manifest.id !== this.input.manifest.id)
					throw new Error("Run manifest identity changed");
