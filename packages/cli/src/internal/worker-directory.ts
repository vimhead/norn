...
export async function prepareRunWorkerDirectory(
	runRoot: string,
): Promise<string> {
	...
	if (!metadata.isDirectory() || metadata.isSymbolicLink())
		throw new Error(
			`Run worker cwd must be a directory, not a symlink: ${directory}`,
		);
	if ((await readdir(directory)).length !== 0)
		throw new Error(`Run worker cwd must be empty: ${directory}`);
