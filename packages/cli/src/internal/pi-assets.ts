...
export function resolvePiAssetsCacheRoot(input: {
	...
	if (input.environment.NORN_PI_CACHE_DIR)
		return resolve(input.environment.NORN_PI_CACHE_DIR);
	if (input.platform === "darwin")
		return join(input.home, "Library", "Caches", "norn", "pi");
...
export async function materializePiAssets(input: {
	readonly archive: string;
	readonly cacheRoot: string;
}): Promise<string> {
	const digest = createHash("sha256").update(input.archive).digest("hex");
	const files: PiAssetFile[] = JSON.parse(
		gunzipSync(Buffer.from(input.archive, "base64")).toString("utf8"),
	);
...
class PiAssetCache {
	...
	constructor(
		private readonly input: {
			...
			readonly expectedFiles: ReadonlyMap<string, Buffer>;
		},
	) {
	...
	async materialize(): Promise<string> {
		...
		if (!(await lstat(this.input.cacheRoot)).isDirectory())
			throw new Error("Pi asset cache root must be a real directory");
		...
		try {
			for (const [path, content] of this.input.expectedFiles) {
				...
				await writeFile(join(staging, ...parts), content, {
					flag: "wx",
					mode: 0o600,
				});
			}
			try {
				...
				if (
					...
					!(await this.validateExistingCache())
				)
					throw error;
			}
			if (!(await this.validateExistingCache()))
				throw new Error("Pi assets disappeared after extraction");
			return this.root;
		} finally {
	...
	private async validateExistingCache(): Promise<boolean> {
		...
		if (seen.size !== this.input.expectedFiles.size)
			throw this.invalidCacheError();
		return true;
	}
	...
	private async validateDirectory(input: {
		...
		for (const entry of await readdir(input.directory, {
			...
			if (entry.isDirectory()) {
				if (
					![...this.input.expectedFiles.keys()].some((file) =>
				...
				if (
					!entry.isFile() ||
					!expected ||
					!(await readFile(absolutePath)).equals(expected)
				)
					throw this.invalidCacheError();
				input.seen.add(path);
			}
	...
	private invalidCacheError(): Error {
		return new Error(
			`Bundled Pi assets are incomplete or modified: ${this.root}. Remove this cache entry and retry.`,
		);
	}
}
