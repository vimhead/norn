...
import { isWorkflowDeclaration, workflow, workflowScope, type NornAnyWorkflowDeclaration, type NornWorkflowContext, type NornWorkflowEntrypoint } from "@vimhead.dev/norn";
...
function createWorkflow({ entrypoint, gate }: { entrypoint: NornWorkflowEntrypoint; gate?: { enabled: true; describe?: () => string } }) {
	return metadata.workflow({ name: "step", entrypoint, args: Type.Object({}), gate, execute: ({ run }) => run.complete() });
}
...
for (const entrypoint of [undefined, null, true, "", "instructions", 42, [], {}, { instructions: undefined }, { instructions: "" }, { instructions: " \n\t " }, { instructions: null }, { instructions: 42 }]) {
	test(`entrypoint declarations and registration reject invalid metadata: ${JSON.stringify(entrypoint)}`, () => {
		assert.throws(() => workflow({ name: "invalid", entrypoint: entrypoint as NornWorkflowEntrypoint, args: Type.Object({}), execute: ({ run }) => run.complete() }), /entrypoint/);
		assert.throws(() => createWorkflow({ entrypoint: entrypoint as NornWorkflowEntrypoint }), /entrypoint/);
		const definition = createWorkflow({ entrypoint: { instructions: "Use to complete the fixture." } });
...
test("internal steps explicitly opt out of default discovery without caller instructions", () => {
	...
	assert.deepEqual(registry.list().map(workflow => workflow.id), [definition.id]);
	assert.deepEqual(definition({}), { type: "next", workflowId: definition.id, args: {} });
});
test("discovery sorts IDs and publishes caller instructions and config ownership", () => {
	const definitions = [
		metadata.workflow({ name: "zebra", entrypoint: { instructions: "Alpha guidance for the last workflow." }, args: Type.Object({}), execute: ({ run }) => run.complete() }),
		metadata.workflow({ name: "middle", entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete() }),
		metadata.workflow({ name: "alpha", entrypoint: { instructions: "Zebra guidance for the first workflow." }, args: Type.Object({}), execute: ({ run }) => run.complete() }),
	];
	assert.deepEqual(definitions[0].entrypoint, { instructions: "Alpha guidance for the last workflow." });
	...
	assert.deepEqual(registry.list({ entrypointsOnly: true }).map(workflow => [workflow.id, workflow.configKey, workflow.isEntrypoint]), [["metadata.alpha", "metadata.alpha", true], ["metadata.zebra", "metadata.zebra", true]]);
...
test("gate fallback is the workflow ID, not caller instructions", async () => {
	const registry = new NornWorkflowRegistry();
	const definition = createWorkflow({ entrypoint: { instructions: "Use to collect records, not as a gate decision request." }, gate: { enabled: true } });
	register(registry, definition);
...
test("gate descriptions remain independent from caller instructions", async () => {
	const registry = new NornWorkflowRegistry();
	const definition = createWorkflow({ entrypoint: { instructions: "Use to collect records." }, gate: { enabled: true, describe: () => "Check the collected evidence before proceeding." } });
	register(registry, definition);
