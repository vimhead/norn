...
async function createProjectFixture(context: TestContext) {
	const projectRoot = await realpath(
		await mkdtemp(join(tmpdir(), "norn-project-config-")),
	);
...
async function executeCli<Output>({
	...
	const execution = executeFile(process.execPath, [cliPath, ...args], {
		cwd,
		timeout: 20000,
		maxBuffer: 1024 * 1024,
	});
...
test("a project file loads its own plugins and config from a nested cwd", async (context) => {
	...
	await writePluginFixture({
		projectRoot,
		relativePath: "workflows/plugin.ts",
		pluginId: "local",
	});
	await writeJsonFixture({
		...
		value: {
			version: 1,
			workflows: ["./workflows/plugin.ts"],
			config: { local: { greeting: "project" } },
		},
	});
	...
	assert.deepEqual(project.config.workflows, ["./workflows/plugin.ts"]);
	assert.deepEqual(
		project.registry
	...
	assert.equal(
		...
		join(projectRoot, "workflows/plugin.ts"),
	);
	assert.equal(project.modules[0].configPath, projectPath);
	assert.deepEqual(
		project.configurations.find((owner) => owner.key === "local")?.config,
		{ greeting: "project" },
	);
	assert.deepEqual(
		project.configFiles.map((file) => file.path),
		[projectPath],
	);
	assert.deepEqual(Object.keys(project.configFiles[0].config).sort(), [
		"config",
		"includes",
		"version",
		"workflows",
	]);
	await assert.rejects(stat(join(projectRoot, "norn.json")), {
		code: "ENOENT",
	});
});
...
test("local plugins compose with globbed and nested reusable configs, each relative to its declaring file", async (context) => {
	...
	for (const [relativePath, pluginId] of [
		["plugin.ts", "local"],
		["packages/a/plugin.ts", "packageA"],
		["packages/b/plugin.ts", "packageB"],
		["shared/plugin.ts", "shared"],
	])
		await writePluginFixture({ projectRoot, relativePath, pluginId });
	await writeJsonFixture({
		path: join(projectRoot, "norn.project.json"),
		value: {
			version: 1,
			workflows: ["./plugin.ts"],
			includes: ["./packages/*/norn.json"],
			config: {
				local: { greeting: "project", options: { replace: "project" } },
				packageA: { greeting: "project" },
			},
		},
	});
	for (const name of ["a", "b"]) {
		await writeJsonFixture({
			path: join(projectRoot, "packages", name, "norn.json"),
			value: {
				workflows: ["./plugin.ts"],
	...
	await writeJsonFixture({
		path: join(projectRoot, "shared/norn.json"),
		value: {
			workflows: ["./plugin.ts"],
			config: {
				local: {
					greeting: "shared",
					options: { keep: "shared", replace: "shared" },
				},
				packageA: { greeting: "shared" },
				shared: { greeting: "shared" },
			},
		},
	});
	...
	assert.deepEqual(
		project.configurations
			...
			.map((owner) => owner.key)
			.sort(),
		["local", "packageA", "packageB", "shared"],
	);
	for (const plugin of project.modules) {
	...
	assert.deepEqual(
		...
		{
			greeting: "project",
			options: { keep: "shared", replace: "project" },
		},
	);
	assert.deepEqual(
		project.configurations.find((owner) => owner.key === "packageA")?.config,
		{ greeting: "project" },
	);
	assert.deepEqual(
		project.configurations.find((owner) => owner.key === "shared")?.config,
		{ greeting: "shared" },
	);
...
test("the nearest project remains the execution boundary", async (context) => {
	...
	await writeJsonFixture({
		path: join(projectRoot, "norn.project.json"),
		value: { workflows: ["./missing-parent-plugin.ts"] },
	});
	...
	await writeJsonFixture({
		path: join(childRoot, "norn.project.json"),
		value: { workflows: ["./plugin.ts"] },
	});
...
for (const plugins of ["./plugin.ts", [42], [""]]) {
	test(`invalid project plugins are rejected rather than discarded: ${JSON.stringify(plugins)}`, async (context) => {
		...
		await writeJsonFixture({
			path: join(projectRoot, "norn.project.json"),
			value: { workflows: plugins },
		});
		await assert.rejects(
			findNornProject(projectRoot),
			(error) =>
				error instanceof AssertError &&
				error.cause.errors.some((issue) =>
					issue.instancePath.startsWith("/workflows"),
				),
		);
	});
}
...
test("project-local plugin config is validated by its manifest", async (context) => {
	...
	await writeJsonFixture({
		...
		value: {
			workflows: ["./plugin.ts"],
			config: { local: { greeting: {} } },
		},
	});
	await assert.rejects(
		loadNornProject(projectRoot),
		(error) =>
			error instanceof NornProjectLoadError &&
			error.diagnostics.some(
				(diagnostic) =>
					diagnostic.stage === "config" &&
					diagnostic.issues.some((issue) => issue.instancePath === "/greeting"),
			),
	);
});
...
test("duplicate plugin ids across project and reusable configs are rejected", async (context) => {
	...
	await assert.rejects(
		loadNornProject(projectRoot),
		/Workflow already registered: duplicate.echo/,
	);
});
...
test("an include cycle back to the project does not register its plugins twice", async (context) => {
	...
	await writeJsonFixture({
		path: projectPath,
		value: { workflows: ["./plugin.ts"], includes: ["./shared/norn.json"] },
	});
	await writeJsonFixture({
		path: join(projectRoot, "shared/norn.json"),
		value: { includes: ["../norn.project.json"] },
	});
	...
	assert.deepEqual(
		project.configFiles.map((file) => file.path),
		[projectPath, join(projectRoot, "shared/norn.json")],
	);
});
...
test("project overrides do not hide conflicts between reusable configs", async (context) => {
	...
	for (const name of ["a", "b"]) {
		await writeJsonFixture({
			path: join(projectRoot, `${name}.json`),
			value: { config: { local: { greeting: name } } },
		});
	}
	await assert.rejects(
		findNornProject(projectRoot),
		/Conflicting Norn reusable config at config.local.greeting/,
	);
});

test(
	"init is self-contained and supports authoring and executing directly afterward",
	{ timeout: 30000 },
	async (context) => {
		const projectRoot = await createProjectFixture(context);
		await executeCli({
		...
		assert.equal(
			(await stat(join(projectRoot, ".norn/runs"))).isDirectory(),
			true,
		);
		await assert.rejects(stat(join(projectRoot, "norn.json")), {
			code: "ENOENT",
		...
		assert.deepEqual(emptyProject.modules, []);
		config.workflows.push("./plugin.ts");
		...
		for (const revision of ["first", "second"]) {
			...
			const { workflows } = await executeCli<NornWorkflowCatalogInfo>({
				cwd: projectRoot,
				args: ["workflows", "list"],
				input: undefined,
			});
			...
			const { run } = await executeCli<{ run: NornRunInfo }>({
				cwd: projectRoot,
				args: ["runs", "start", "local.echo"],
				input: { args: { value: "saved" } },
			});
			const finished = await executeCli<{ run: NornRunInfo }>({
				cwd: projectRoot,
				args: ["runs", "wait", run.id],
				input: undefined,
			});
			assert.equal(finished.run.status, "completed");
			assert.deepEqual(finished.run.outcome?.metadata?.data, {
				value: "saved",
				revision,
			});
		}
		await assert.rejects(
			...
			(error) => /Norn project already exists/.test(readProcessStdout(error)),
		);
...
for (const siblingTiming of ["before", "after"]) {
	test(
		`a sibling norn.json created ${siblingTiming} init is not implicitly included`,
		{ timeout: 10000 },
		async (context) => {
			const projectRoot = await createProjectFixture(context);
			const siblingPath = join(projectRoot, "norn.json");
			if (siblingTiming === "before")
				await writeFile(
					siblingPath,
					"invalid JSON that must not be read or changed",
				);
			await executeCli({
				cwd: projectRoot,
				args: ["project", "init"],
