...
import {
	SharedState,
	type SharedStateAccess,
} from "../examples/shared-state/shared-state.ts";
...
test(
	"independent state processes preserve every field without seeding defaults",
	{ timeout: 15000 },
	async (context) => {
					...
					new Promise<void>((resolve, reject) => {
						const child = spawn(
							...
							[
								...
								fileURLToPath(
									new URL("./fixtures/file-lock-worker.ts", import.meta.url),
								),
								root,
								"state",
								`${index}`,
		...
		try {
			assert.equal(
				database.prepare("SELECT COUNT(*) AS count FROM state").get()?.count,
				48,
			);
		} finally {
			database.close();
		...
		assert.equal(
			await state.getOptional({
				id: "missing",
				schema: Type.String({ default: "not invented" }),
			}),
			undefined,
		);
		await assert.rejects(
			state.get({ id: "missing", schema: Type.String() }),
...
test("state tools delegate to a supplied interface and respect cancellation", async (context) => {
	...
	const state: SharedStateAccess = {
		get: stored.get.bind(stored),
		getOptional: stored.getOptional.bind(stored),
		set: stored.set.bind(stored),
	};
	...
	const result = await get.execute(
		"read",
		{ key: field.id, offset: 0, limit: 10000 },
		undefined,
		undefined,
		{} as never,
	);
	assert.deepEqual(JSON.parse((result.details as { text: string }).text), {
		isSet: true,
		value: "saved",
	});
...
test("checkpoint rollback restores a closed workspace database", async (context) => {
	...
	try {
		assert.equal(await reopened.get(field), "before");
	} finally {
		reopened.close();
	}
});
...
test("state tools scope access, validate schemas and paginate large values", async (context) => {
	...
	const tools = createStateTools({
		state,
		fields: [{ field: visible, access: "read" }],
	});
	const execute = async (name: string, args: object) =>
		tools
			.find((tool) => tool.name === name)!
			.execute("call", args, undefined, undefined, {} as never);
	...
	await assert.rejects(
		execute("norn_state_get", { key: "hidden", offset: 0, limit: 10000 }),
		/not attached/,
	);
	await assert.rejects(
		execute("norn_state_set", { key: "visible", value: "changed" }),
		/not attached/,
	);
	const first = await execute("norn_state_get", {
		key: "visible",
		offset: 0,
		limit: 10000,
	});
	...
	const writerTools = createStateTools({
		state,
		fields: [{ field: visible, access: "write" }],
	});
	await assert.rejects(
		writerTools
			.find((tool) => tool.name === "norn_state_set")!
			.execute(
				"call",
				{ key: "visible", value: 3 },
				undefined,
...
test("workflows reopen their own workspace state across transitions and gate resume", async (context) => {
	...
	const start = scope.workflow({
		name: "start",
		entrypoint: { instructions: "Persist a value before a gate." },
		args: Type.Object({}),
		async execute({ paths }) {
			const store = await SharedState.open({
				path: join(paths.workspace, "state.sqlite"),
				create: true,
			});
			try {
				await store.set(field, 7);
				return finish({ decision: "reject" });
			} finally {
				store.close();
			}
		},
	...
	const finish = scope.workflow({
		...
		gate: {
			enabled: true,
			fields: ["decision"],
			describe: () => "Accept the persisted value.",
		},
		async execute({ paths, run }) {
			...
			try {
				return run.complete({ data: { value: await store.get(field) } });
			} finally {
				store.close();
			}
		},
	...
	const completed = await resumed.resumeWorkflow(
		join(root, ".norn", "runs", interrupted.id),
		{ decision: "accept" },
	);
