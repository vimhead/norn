...
test(
	"package builds isolate declaration roots and fail without publishing declarations for invalid source",
	{ timeout: 30_000 },
	async (context) => {
		...
		await writeFile(
			...
			'export type { Options } from "./nested/options.ts";\nexport const count: number = 1;\n',
		);
		await writeFile(
			join(sourceRoot, "nested/options.ts"),
			"export type Options = { label: string };\n",
		);
		await writeFile(
			join(sourceRoot, "bun/entry.ts"),
			'export const invalid: number = "excluded binary source";\n',
		);
		await writeFile(
			join(root, "packages/unrelated/src/index.ts"),
			'export const invalid: number = "unrelated package";\n',
		);
		...
		assert.match(
			await readFile(join(outputRoot, "nested/options.d.ts"), "utf8"),
			/label: string/,
		);
		assert.match(
		...
		assert.deepEqual(
			(await readdir(join(root, "dist"))).filter((path) =>
		...
		await assert.rejects(access(join(outputRoot, "index.d.ts")), {
			code: "ENOENT",
		});
		assert.deepEqual(
			(await readdir(join(root, "dist"))).filter((path) =>
