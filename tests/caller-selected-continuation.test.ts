...
async function copyContinuationExample(context: TestContext) {
	...
	await cp(
		new URL("../examples/caller-selected-continuation/", import.meta.url),
		root,
		{ recursive: true },
	);
	const input: { args: NornWorkflowArgsInput<typeof write> } = JSON.parse(
		await readFile(join(root, "input.json"), "utf8"),
	);
...
test("the copied continuation example exposes its contribution contract and completes in the supplied consumer", async (context) => {
	...
	assert.deepEqual(catalogue.workflows.map((workflow) => workflow.id).sort(), [
		"greetingConsumer.saveJson",
		"greetingConsumer.saveText",
		"greetingProducer.write",
	]);
	const producer = await client.workflows.inspect("greetingProducer.write");
	expect(producer.workflow?.argsSchema).toHaveProperty(
	...
	expect(consumer.workflow?.argsSchema).toHaveProperty("required", [
		"batchId",
		"resultPath",
		"summary",
	]);
	...
	assert.deepEqual(finished.outcome?.metadata?.data, {
		batchId: "batch-17",
		format: "json",
		greetingPath: "greeting.txt",
		deliveryPath: "delivery.json",
	});
	assert.equal(
	...
	assert.deepEqual(
		JSON.parse(
			await readFile(join(finished.paths.workspace, "delivery.json"), "utf8"),
		),
		{
			batchId: "batch-17",
	...
	assert.ok(
		(await client.runs.checkpoints(started.id)).some(
			(checkpoint) =>
				checkpoint.message ===
				"transition: greetingProducer.write -> greetingConsumer.saveJson",
		),
	);
});
...
test("changing only the caller reference selects another consumer and forwards its batch ID", async (context) => {
	...
	const started = await client.runs.start({
		...
		args: {
			...args,
			next: {
				workflow: "greetingConsumer.saveText",
				forwardArgs: { batchId: "batch-99" },
			},
		},
	});
	const finished = await client.runs.wait(started.id);
	...
	assert.equal(
		await readFile(join(finished.paths.workspace, "delivery.txt"), "utf8"),
		"batch-99: Hello, Ada!\n",
	);
});
...
test("a producer contribution cannot complete delivery without the consumer's required caller context", async (context) => {
	...
	const started = await client.runs.start({
		...
		args: {
			...args,
			next: { workflow: "greetingConsumer.saveJson", forwardArgs: {} },
		},
	});
	const finished = await client.runs.wait(started.id);
	...
	await assert.rejects(
		readFile(join(finished.paths.workspace, "delivery.json")),
		{ code: "ENOENT" },
	);
});
