...
async function project(context: TestContext, files: Record<string, string>, modules: string[]) {
	...
	await writeFile(join(root, "norn.project.json"), JSON.stringify({ version: 1, workflows: modules, config: { reports: { path: "shared" }, "reports.summarize": { path: "local" } } }));
	return root;
}
...
test("authoring names produce literal IDs and string transitions use exact IDs", async context => {
	...
	const standalone = workflow({ name: "save", entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete({ summary: "standalone" }) });
	const reports = workflowScope({ name: "reports" });
	const audits = workflowScope({ name: "audits" });
	const report = reports.workflow({ name: "save", entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete({ summary: "report" }) });
	const audit = audits.workflow({ name: "save", entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete({ summary: "audit" }) });
	const route = reports.workflow({ name: "route", entrypoint: false, args: Type.Object({ next: Type.String() }), execute: ({ args, run }) => run.next(args.next, {}) });
...
for (const name of [undefined, null, 42, "", " \n\t ", ".", "reports.save", ".save", "save.", "reports..save"]) {
	test(`workflow and scope names reject empty, non-string, or dotted values: ${JSON.stringify(name)}`, () => {
		...
		assert.throws(() => workflow({ name: name as string, entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete() }), /Invalid workflow or scope name/);
		const reports = workflowScope({ name: "reports" });
		assert.throws(() => reports.workflow({ name: name as string, entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete() }), /Invalid workflow or scope name/);
	});
}
...
test("a scope spans registered modules with codecs and independent local configuration", async context => {
	...
	const inspection = await inspectNornWorkflow({ cwd: root, workflowId: "reports.summarize" });
	...
	assert.deepEqual(inspection.workflow?.source, { path: join(root, "summarize.ts"), configPath: join(root, "norn.project.json") });
	const engine = new NornEngine({ cwd: root, config: loaded.projectConfig });
	engine.registerWorkflows(loaded.definitions);
	const result = await engine.runWorkflow(loaded.registry.workflowById("reports.summarize")!, {}, undefined);
	assert.equal(result.status, "completed");
	assert.deepEqual(result.metadata?.data, { path: "local:SHARED", shared: "SHARED" });
});

test("scope identity survives unrelated import failures and a new load sees source edits", async context => {
	const root = await project(context, { "scope.ts": scopeSource, "summarize.ts": summarizeSource, "save.ts": saveSource, "broken.ts": 'throw new Error("broken sibling")' }, ["./summarize.ts", "./broken.ts", "./save.ts"]);
	const incomplete = await discoverNornProject(root);
	assert.equal(incomplete.isComplete, false);
	assert.deepEqual(incomplete.workflows.map(entry => entry.id), ["reports.save", "reports.summarize"]);
	assert.equal(incomplete.diagnostics.length, 1);
	await writeFile(join(root, "broken.ts"), "export default [];");
	await writeFile(join(root, "scope.ts"), scopeSource.replace("path.toUpperCase()", "path.toLowerCase()"));
	const loaded = await loadNornProject(root);
	assert.equal(loaded.configurations.filter(owner => owner.key === "reports").length, 1);
	assert.deepEqual(loaded.configurations.find(owner => owner.key === "reports")?.config, { path: "shared" });
});

test("imported but unregistered targets fail execution instead of auto-registering", async context => {
	const root = await project(context, { "scope.ts": scopeSource, "summarize.ts": summarizeSource, "save.ts": saveSource }, ["./summarize.ts"]);
	const loaded = await loadNornProject(root);
	assert.deepEqual(loaded.registry.list().map(entry => entry.id), ["reports.summarize"]);
	const engine = new NornEngine({ cwd: root, config: loaded.projectConfig });
	engine.registerWorkflows(loaded.definitions);
	await assert.rejects(engine.runWorkflow(loaded.definitions[0], {}, undefined), /Unknown next workflow: reports.save/);
});
...
export default [workflow({ name: "direct", entrypoint: false, args: Type.Object({}), execute }), scope.workflow({ name: "gate", entrypoint: false, args: Type.Object({}), gate: { enabled: true, describe: execute }, execute })];` }, ["./mixed.ts"]);
	const found = await discoverNornProject(root);
	assert.equal(found.isComplete, true);
	assert.deepEqual(found.workflows.map(entry => entry.id), ["direct", "other.gate"]);
	assert.equal((await inspectNornWorkflow({ cwd: root, workflowId: "other.gate" })).workflow?.gate?.enabled, true);
});

for (const malformed of ['[valid, {}]', '[valid, ,]', '{ valid }']) {
	test(`malformed registration excludes the entire module: ${malformed}`, async context => {
		const root = await project(context, { "invalid.ts": `import { workflow } from "@vimhead.dev/norn"; import { Type } from "typebox"; const valid = workflow({ name: "valid", entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete() }); export default ${malformed};` }, ["./invalid.ts"]);
		const found = await discoverNornProject(root);
		assert.equal(found.isComplete, false);
...
test("configuration ownership collisions and independent scope declarations fail registration", async context => {
	...
	const first = reports.workflow({ name: "first", entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete() });
	const second = wrong.workflow({ name: "second", entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete() });
	const standalone = workflow({ name: "reports", entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete() });
	...
	const identical = duplicate.workflow({ name: "identical", entrypoint: false, args: Type.Object({}), execute: ({ run }) => run.complete() });
...
test("incompatible saved runs fail resume and rollback without modifying saved files", async context => {
	const root = await project(context, {}, []);
	const gated = workflow({ name: "gate", entrypoint: false, args: Type.Object({ approved: Type.Boolean() }), gate: { enabled: true, fields: ["approved"] }, execute: ({ run }) => run.complete() });
	...
	const resumeRequest = JSON.stringify({ version: 1, type: "resume", id: result.id, params: { approved: true }, createdAt: new Date().toISOString() });
	await writeFile(join(runRoot, "resume-request.json"), resumeRequest);
	const before = await readFile(path, "utf8");
	assert.equal((await getRunInfo(runRoot)).version, 1);
	assert.deepEqual((await getRunInfo(runRoot)).interruption?.args, { approved: false });
	assert.equal((await listRuns(root)).length, 1);
	await assert.rejects(engine.resumeWorkflow(runRoot, { approved: true }), /Incompatible saved run/);
	await assert.rejects(engine.rollbackRun(runRoot, checkpoints[0].id), /Incompatible saved run/);
	assert.equal(await readFile(path, "utf8"), before);
	assert.equal(await readFile(join(runRoot, "resume-request.json"), "utf8"), resumeRequest);
	assert.equal(await readFile(join(runRoot, "current/state.json"), "utf8"), '{"legacy":"retained"}');
});
