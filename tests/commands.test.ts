...
async function createFixture(context: TestContext, signal?: AbortSignal) {
	...
	vi.spyOn(logger, "record").mockImplementation(async (event) => {
		events.push(event);
	});
...
function printCommand(text: string) {
	return [
		process.execPath,
		"-e",
		`process.stdout.write(${JSON.stringify(text)}); process.stderr.write(${JSON.stringify(`error:${text}`)})`,
	] as const;
}

test("repeated labels retain each invocation's stdout and stderr", async (context) => {
	const fixture = await createFixture(context);
	const first = await fixture.runner.run({
		label: "repeat",
		cwd: fixture.cwd,
		command: printCommand("first"),
	});
	const second = await fixture.runner.run({
		label: "repeat",
		cwd: fixture.cwd,
		command: printCommand("second"),
	});
	...
	const started = fixture.events.filter(
		(event) => event.type === "command.started",
	);
...
test("concurrent commands and new runner instances cannot collide on labels", async (context) => {
	...
	const inputs = Array.from({ length: 8 }, (_, index) => ({
		label: index % 2 ? "a/b" : "a_b",
		cwd: fixture.cwd,
		command: printCommand(String(index)),
	}));
	const results = await Promise.all(
		inputs.map((input) => fixture.runner.run(input)),
	);
	const nextRunner = new NornCommandRunner({
		logs: fixture.logs,
	...
	results.push(
		await nextRunner.run({
			label: "a_b",
			cwd: fixture.cwd,
			command: printCommand("8"),
		}),
	);
	assert.equal(
		new Set(
			results.flatMap((result) => [result.stdoutLog.id, result.stderrLog.id]),
		).size,
		18,
	);
	for (const [index, result] of results.entries())
		assert.equal(await fixture.logs.read(result.stdoutLog), String(index));
});

for (const cancellation of ["timeout", "abort"]) {
	test(
		`${cancellation} escalates when a child ignores SIGTERM`,
		...
		async (context) => {
			...
			const resultPromise = fixture.runner.run({
				label: "stubborn",
				cwd: fixture.cwd,
				command: [process.execPath, "-e", script, pidPath],
				timeoutMs: cancellation === "timeout" ? 800 : 2000,
			});
			...
			assert.ok(
				pid !== undefined && pid > 0,
				"child registered its signal handler",
			);
			...
			assert.ok(
				...
				"a second cancellation must not postpone escalation",
			);
			pid = undefined;
		},
	);
