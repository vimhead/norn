...
import { workflow, workflowScope, workflowRefSchema, type NornRun, type NornWorkflowArgs } from "@vimhead.dev/norn";
import { Type } from "typebox";
import { AssertError, Value } from "typebox/value";
import { test, type TestContext } from "vitest";
import { NornEngine } from "../packages/cli/src/internal/engine.ts";

const declarationsScope = workflowScope({ id: "callables" });
...
args: Type.Object({
			...
			next: Type.Object({
				success: workflowRefSchema({ args: Type.Object({ count: Type.String() }) }),
				failure: workflowRefSchema({ args: Type.Object({ reason: Type.String() }) }),
			}),
		}),
execute: ({ args: args, run: run }) => {
			switch (args.outcome) {
				case "success": return args.next.success({ count: "2" });
				case "failure": return args.next.failure({ reason: "rejected" });
				case "custom": return declarations_custom({ reason: "manual review" });
				...
				case "leak": return declarations_sink({ leaked: args.next.success });
			}
		}
});
...
const plugin = [declarations_source, declarations_success, declarations_failure, declarations_custom, declarations_sink];

function input(outcome: "success" | "failure" | "custom" | "dynamic" | "throw" | "leak") {
	return { outcome, next: {
		success: { workflow: declarations_success.id, forwardArgs: { batchId: "success-batch", count: "stale" } },
		failure: { workflow: declarations_failure.id, forwardArgs: { batchId: "failure-batch" } },
	} };
}
...
for (const [outcome, expected] of [
	...
	test(`${outcome} routing uses the existing scheduler and target contract`, async context => {
		...
		const result = await engine.runWorkflow(declarations_source, input(outcome), undefined);
...
test("named failure handlers do not catch unhandled exceptions", async context => {
	const { engine } = await fixture(context, "auto");
	await assert.rejects(engine.runWorkflow(declarations_source, input("throw"), undefined), /Unhandled producer failure/);
});
...
test("reference invocation does not bypass target lookup or full input validation", async context => {
	...
	await assert.rejects(engine.runWorkflow(declarations_source, { ...missing, next: { ...missing.next, success: { workflow: "missing.target", forwardArgs: {} } } }, undefined), /Unknown next workflow/);
	const invalid = { ...missing, next: { ...missing.next, success: { workflow: "callables.success", forwardArgs: { batchId: {} } } } };
	await assert.rejects(engine.runWorkflow(declarations_source, invalid, undefined), AssertError);
});

test("decoded functions cannot silently disappear at persistence boundaries", async context => {
	const { engine } = await fixture(context, "auto");
	await assert.rejects(engine.runWorkflow(declarations_source, Value.Decode(declarations_source.args, input("success")), undefined), AssertError);
	await assert.rejects(engine.runWorkflow(declarations_source, input("leak"), undefined), AssertError);
});

function verifyBranchTypes(run: NornRun, args: NornWorkflowArgs<typeof declarations_source>) {
