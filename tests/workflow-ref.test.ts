...
for (const reference of ["refs.finish", target.id]) {
	test("workflow references decode into contribution-taking transition builders", () => {
		...
		const options: NornWorkflowRefSchemaOptions<typeof contributions> = { args: contributions };
		const schema = workflowRefSchema(options);
		assert.deepEqual(Value.Decode(workflowRefSchema({}), reference)({}), { type: "next", workflowId: "refs.finish", args: {} });
		...
		const input = { workflow: reference, forwardArgs: { task: "retain", report: "stale" } };
		const next = Value.Decode(schema, input);
		assert.deepEqual(next({ report: "done" }), { type: "next", workflowId: reference, args: { task: "retain", report: "done" } });
		assert.deepEqual(next({ report: "again" }).args, { task: "retain", report: "again" });
...
test("nullable reference inputs retain their required property and input union", () => {
	...
	expect(inspected).toHaveProperty("properties.next.anyOf.0.anyOf.1.required", ["workflow", "forwardArgs"]);
});
...
test("inspection advertises encoded contributions and invocation does not decode them", () => {
	...
	const reference = Type.With(workflowRefSchema({ args: contributions }), { description: "Continue with the collected records" });
	const schema = Type.Object({ next: reference });
	const inspected = inspectSchema(schema);
	expect(inspected).toHaveProperty("properties.next.x-norn-workflow-ref.contributedArgsSchema", inspectSchema(contributions));
	...
	expect(inspected).toHaveProperty("properties.next.anyOf.1.properties.forwardArgs.patternProperties", { "^.*$": {} });
	expect(inspected).toHaveProperty("properties.next.x-norn-workflow-ref.contributedArgsSchema.required", ["records", "count"]);
...
test("references without contributions advertise an empty object schema", () => {
	...
	expect(inspectSchema(reference)).toHaveProperty("x-norn-workflow-ref.contributedArgsSchema", inspectSchema(Type.Object({})));
	assert.deepEqual(Value.Decode(reference, "refs.finish")({}), { type: "next", workflowId: "refs.finish", args: {} });
});

test("optional, nullable and array references retain annotations and callable codecs", () => {
	const reference = workflowRefSchema({ args: Type.Object({ report: Type.String() }) });
	...
	for (const path of ["properties.optional", "properties.nullable.anyOf.0", "properties.many.items"]) {
		expect(inspected).toHaveProperty(`${path}.x-norn-workflow-ref.contributedArgsSchema`, inspectSchema(Type.Object({ report: Type.String() })));
	}
...
test("recursive contributions use native TypeBox definitions", () => {
	...
	const reference = workflowRefSchema({ args: contributions });
	expect(inspectSchema(reference)).toHaveProperty("x-norn-workflow-ref.contributedArgsSchema", inspectSchema(contributions));
...
test("object-valued unions, records, intersections and root codecs can contribute", () => {
	const union = workflowRefSchema({ args: Type.Union([Type.Object({ kind: Type.Literal("ok"), report: Type.String() }), Type.Object({ kind: Type.Literal("error"), reason: Type.String() })]) });
	assert.deepEqual(Value.Decode(union, "target")({ kind: "error", reason: "failed" }).args, { kind: "error", reason: "failed" });
	const record = workflowRefSchema({ args: Type.Record(Type.String(), Type.Integer()) });
	assert.deepEqual(Value.Decode(record, "target")({ first: 1 }).args, { first: 1 });
	const intersection = workflowRefSchema({ args: Type.Intersect([Type.Object({ left: Type.String() }), Type.Object({ right: Type.Integer() })]) });
	assert.deepEqual(Value.Decode(intersection, "target")({ left: "x", right: 2 }).args, { left: "x", right: 2 });
	const codec = workflowRefSchema({ args: Type.Decode(Type.Object({ text: Type.String() }), value => value.text) });
	assert.deepEqual(Value.Decode(codec, "target")({ text: "encoded" }).args, { text: "encoded" });
	const unknown = Value.Decode(workflowRefSchema({ args: Type.Unknown() }), "target");
...
test("inspection traverses schema positions, not defaults, examples or property-name maps", () => {
	const payload = { "x-norn-workflow-ref": { contributedArgsSchema: { type: "bigint" } } };
...
for (const input of [undefined, null, "", 3, {}, { workflow: "refs.finish" }, { workflow: "", forwardArgs: {} }, { workflow: "refs.finish", forwardArgs: null }, { workflow: target, forwardArgs: {} }]) {
