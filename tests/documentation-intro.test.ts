...
import {
	...
	type NornDocumentationLocation,
...
const documentation: NornDocumentationLocation = {
	...
	assetDigest: null,
	paths: {
		root: "/installation/norn",
		readme: "/installation/norn/README.md",
		index: "/installation/norn/docs/README.md",
		docs: "/installation/norn/docs",
		examples: "/installation/norn/examples",
	},
};
...
test("intro is compact, deterministic, and points to the index instead of copying topic manuals", () => {
	const input = {
		documentation,
		invocation: ["/runtime/node", "/installation/norn/bin/norn.mjs"] as const,
	};
	...
	assert.match(
		intro,
		/agent-driven and code-driven TypeScript workflows with the Norn SDK/,
	);
	assert.match(
		intro,
		/Norn agents are powered by the bundled, open-source and extensible Pi coding agent/,
	);
	assert.match(
		intro,
		/authored, exercised, repaired, and reused during a task/,
	);
	assert.match(intro, /Norn is optional/);
	assert.match(intro, /build commit: unknown/);
	for (const key of ["readme", "index", "docs", "examples"] as const)
		assert.ok(intro.includes(JSON.stringify(documentation.paths[key])));
	assert.ok(intro.includes('["workflows", "list"]'));
	assert.ok(intro.includes('["workflows", "inspect", "<workflow-id>"]'));
	assert.ok(
		!intro.includes("agents.md"),
		"topic-to-page mapping stays in the documentation index",
	);
	assert.ok(
		!intro.includes("workflowScope"),
		"no authoring manual is embedded",
	);
});
...
test("runtime argv and paths retain whitespace, quotes, backslashes and newlines without shell quoting", () => {
	...
	const unusualDocumentation = {
		...documentation,
		commit: "a".repeat(40),
		paths: { ...documentation.paths, index: '/docs/quoted "path"\n/README.md' },
	};
	const intro = renderNornDocumentationIntro({
		documentation: unusualDocumentation,
		invocation,
	});
	const runtimeLine = intro.match(/^Runtime argv .*: (.+)$/m);
	const indexLine = intro.match(
		/^Documentation index and topic routing: (.+)$/m,
	);
...
test("source CLI produces intro outside a valid project and its invocation selects the same runtime", async (context) => {
	...
	const options = {
		cwd: root,
		env: { ...process.env, NORN_DOCS_CACHE_DIR: cacheRoot },
		timeout: 30_000,
	};
	const result = JSON.parse(
		(await execute(process.execPath, [cli, "docs", "intro"], options)).stdout,
	);
	assert.deepEqual(Object.keys(result), ["intro"]);
	const invocation = JSON.parse(
		result.intro.match(/^Runtime argv .*: (.+)$/m)[1],
	);
	...
	const inspected = JSON.parse(
		(
			await execute(
				invocation[0],
				[...invocation.slice(1), "docs", "inspect"],
				options,
			)
		).stdout,
	).documentation;
	assert.equal(
		result.intro,
		renderNornDocumentationIntro({ documentation: inspected, invocation }),
	);
	...
	const metadata = JSON.parse(
		(
			await execute(
				process.execPath,
				[cli, "commands", "inspect", "docs.intro"],
				options,
			)
		).stdout,
	).command;
	...
	assert.match(
		(await execute(process.execPath, [cli, "help", "docs", "intro"], options))
			.stdout,
		/norn docs intro/,
	);
	await assert.rejects(
		execute(process.execPath, [cli, "docs", "intro", "unexpected"], options),
		(error) => {
