...
test(
	"real npm tarballs provide public imports, exact dependencies, offline docs and an independent Pi adapter outside the checkout",
	{ timeout: 240_000 },
	async (context) => {
		const root = await mkdtemp(join(tmpdir(), "norn-packages-"));
		context.onTestFinished(() => rm(root, { recursive: true, force: true }));
		const tarballs = join(root, "tarballs");
		const packages = await packReleasePackages({
			workspaceRoot,
			outputDirectory: tarballs,
		});
		assert.deepEqual(
			packages.map((pkg) => pkg.name),
			["@vimhead.dev/norn", "@vimhead.dev/norn-cli", "@vimhead.dev/pi-norn"],
		);
		...
		await execute(
			...
			[
				...
				"--no-audit",
				"--no-fund",
				...packages.map((pkg) => join(tarballs, pkg.filename)),
				"@types/node@26.2.0",
			],
		...
		for (const packageRoot of [runtimeRoot, adapterRoot, libraryRoot]) {
			...
			for (const file of await readdir(join(packageRoot, "dist"), {
				...
				assert.ok(
					!content.includes("@vimhead.dev/norn-core"),
					`${manifest.name}: private dependency escaped into ${file}`,
				);
			}
		}
		const manifest = JSON.parse(
		...
		assert.equal(
			manifest.dependencies["@vimhead.dev/norn"],
			packages[0].version,
		);
		...
		const environment = {
			...process.env,
			NORN_AGENT_DIR: join(root, "agent"),
		...
		assert.equal(
			documentation.paths.index,
			join(canonicalRuntime, "assets/docs/README.md"),
		);
		for (const path of await readdir(documentation.paths.root, {
			recursive: true,
		})) {
			if (!path.endsWith(".md")) continue;
			const content = await readFile(
				join(documentation.paths.root, path),
				"utf8",
			);
			for (const match of content.matchAll(/\]\(([^)]+)\)/g)) {
				...
				if (link && !/^[a-z]+:/i.test(link))
					await access(
						resolve(dirname(join(documentation.paths.root, path)), link),
					);
			}
		}
		...
		assert.deepEqual(invocation, [
			process.execPath,
			join(canonicalRuntime, "bin/norn.mjs"),
		]);
...
`;
		const result = await execute(
			...
			{ cwd: consumer, env: environment, timeout: 60_000 },
		);
