...
const incrementInput = Type.Decode(
	Type.Object({ count: Type.String() }),
	(input) => ({ count: Number(input.count) + 1 }),
);
const continuation = workflowRefSchema({
	args: Type.Object({ report: Type.String() }),
});
const step = workflow({
	name: "step",
	entrypoint: { instructions: "Exercise native TypeBox contracts." },
	args: incrementInput,
	gate: {
		enabled: true,
		fields: ["count"],
		describe: ({ args }) => `Count ${args.count}`,
	},
	execute(context) {
...
test("native inference preserves encoded input, decoded output and standalone IDs", () => {
	...
	expectTypeOf<NornWorkflowArgsInput<typeof step>>().toEqualTypeOf<{
		count: string;
	}>();
	expectTypeOf<NornWorkflowArgs<typeof step>>().toEqualTypeOf<{
		count: number;
	}>();
});
function verifyAuthoringTypes(
	run: NornRun,
	reference: StaticDecode<typeof continuation>,
) {
	const definition = {
		name: "entrypoint-contract",
		args: Type.Object({}),
		execute: ({ run }: { run: NornRun }) => run.complete(),
	};
	workflow({ ...definition, entrypoint: false });
	workflow({
		...definition,
		entrypoint: { instructions: "Complete without effects." },
	});
	...
	workflow({
		...
		gate: { enabled: true, fields: ["missing"] },
	...
	scope.workflow({
		...
		entrypoint: false,
		args: Type.Object({}),
		execute({ config, scope, paths, run }) {
...
test("recursive workflows retain inferred context with a result annotation", async (context) => {
	const repeat = workflow({
		name: "repeat",
		entrypoint: false,
		args: Type.Object({ count: Type.Integer() }),
		execute({ args, run }): WorkflowResult {
			return args.count ? repeat({ count: args.count - 1 }) : run.complete();
		},
	});
	const engine = new NornEngine({ cwd: await fixture(context) });
	engine.registerWorkflows([repeat]);
	assert.equal(
		(await engine.runWorkflow(repeat, { count: 2 }, undefined)).status,
		"completed",
	);
});
...
test("native defaults remain annotations while codecs infer their output independently", () => {
	const schema = Type.Decode(
		Type.Object({ count: Type.Optional(Type.Integer({ default: 3 })) }),
		(input) => ({ count: input.count ?? 3 }),
	);
...
test("gates and resumed executions decode inputs without persisting transformed values", async (context) => {
	...
	assert.ok(
		(await readFile(join(runRoot, "current/run-state.json"), "utf8")).includes(
			'"2"',
		),
	);
});
...
test("workflow arguments use conversion, defaults and cleaning", async (context) => {
	const decode = workflow({
		...
		execute: ({ args, run }) => run.complete({ data: args }),
	});
	const engine = new NornEngine({ cwd: await fixture(context) });
	engine.registerWorkflows([decode]);
	for (const [input, expected] of [
		[{}, 3],
		[{ count: "4", extra: "removed" }, 4],
	] as const) {
...
test("workflow and scope configurations decode and override independently, including gates", async (context) => {
	...
	const summarize = reports.workflow({
		name: "summarize",
		entrypoint: false,
		...
		execute({ config, scope, run }) {
			return run.complete({ data: { local: config, shared: scope.config } });
		},
	});
	const cwd = await fixture(context);
	const engine = new NornEngine({
		...
		gateMode: "pause",
		config: {
			reports: { count: "2", label: "shared" },
			"reports.summarize": { count: "5", label: "local" },
		},
	});
	...
	const result = await engine.runWorkflow(
		...
		{
			configOverride: {
				reports: { label: "shared override" },
				"reports.summarize": { label: "local override" },
			},
		},
	);
	assert.equal(result.status, "interrupted");
	assert.equal(result.interruption?.description, "6/3");
	const completed = await engine.resumeWorkflow(
		join(cwd, ".norn/runs", result.id),
		{ approved: true },
	);
	assert.equal(completed.status, "completed");
	assert.deepEqual(completed.metadata?.data, {
		local: { count: 6, label: "local override" },
		shared: { count: 3, label: "shared override" },
	});
});
...
test("example shared storage validates values without decoding codecs or initializing defaults", async (context) => {
	...
	const field = {
		id: "count",
		schema: Type.Decode(Type.String(), (text) => Number(text) + 1),
	};
	await state.set(field, "2");
	assert.equal(await state.get(field), "2");
	await assert.rejects(
		state.set({ id: "json", schema: Type.Unknown() }, { invalid: new Date() }),
	);
	assert.equal(
		await state.getOptional({
			id: "missing",
	...
	await assert.rejects(
		state.set(
			{ id: "integer", schema: Type.Integer() },
...
test("response capture applies native codecs once and retains validation failures", () => {
	...
	assert.deepEqual(collector.capture("run", "step", { count: "2" }), {
		label: "step",
		response: { count: 3 },
	});
	assert.deepEqual(collector.get("run"), {
		called: true,
		response: { count: 3 },
	});
	dispose();
	...
	assert.throws(() =>
		collector.capture("invalid", "step", { count: "not a number" }),
	);
	assert.deepEqual(collector.get("invalid"), {
		called: false,
		response: undefined,
	});
});
