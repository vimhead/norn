...
test(
	"detached CLI resumes wait for the new execution, including slow plugin loading",
	{ timeout: 45000 },
	async (context) => {
...
`,
		...
		const executeRun = (args: readonly string[], input?: unknown) =>
			cli<{ run: NornRunInfo }>({ cwd, args, input });
		const started = await executeRun(["runs", "start", "cli.decide"], {
			args: { answer: false },
		});
		const runId = started.run.id;
		assert.equal(
			(await executeRun(["runs", "wait", runId])).run.status,
			"interrupted",
		);
		const { checkpoints } = await cli<{ checkpoints: NornRunCheckpoint[] }>({
			cwd,
			args: ["runs", "checkpoints", runId],
		});
		const resumed = await executeRun(["runs", "resume", runId], {
			args: { answer: true },
		});
		...
		assert.equal(
			(await executeRun(["runs", "rollback", runId, checkpoints[0].id])).run
				.status,
			"pendingResume",
		);
		assert.equal(
			(await executeRun(["runs", "resume", runId])).run.status,
			"running",
		);
		assert.equal(
			(await executeRun(["runs", "wait", runId])).run.status,
