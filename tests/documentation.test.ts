...
} from "../packages/cli/src/documentation.ts";
import {
	validateDocumentationBundle,
	hashDocumentationBundle,
} from "../packages/cli/src/internal/documentation-bundle.ts";
...
const build: NornBuildInfo = {
	...
	releaseTag: "tip",
	assetName: "norn-test",
	checksumAssetName: "norn-test.sha256",
};
const bundle = {
	version: "0.1.0",
	files: [
		...
		{
			path: "docs/README.md",
			content: "[Example](../examples/demo/plugin.ts)\n",
		},
		{
			path: "examples/demo/plugin.ts",
			content: "export const greeting = 'Héllo!';\n",
		},
	],
};
...
async function createFixture(context: TestContext) {
	...
	return {
		root,
		input: {
			source: { kind: "embedded" as const, bundle },
			build,
...
test("local documentation resolves from the installation, without creating a cache", async (context) => {
	...
	const input = {
		...fixture.input,
		build: { ...build, version },
		source: { kind: "local" as const, root: packageRoot },
	};
	...
	const unknown = await resolveNornDocumentation({
		...input,
		build: { ...input.build, commit: null },
	});
	assert.equal(unknown.commit, null);
	await assert.rejects(
		resolveNornDocumentation({
			...input,
			build: { ...input.build, version: "mismatched" },
		}),
		/Local documentation version does not match/,
	);
});
...
test("embedded content is exact, cache reuse performs no rewrites, and links stay relative", async (context) => {
	...
	for (const file of bundle.files)
		assert.equal(
			await readFile(join(first.paths.root, file.path), "utf8"),
			file.content,
		);
	...
	assert.deepEqual(await readdir(input.cacheRoot), [
		first.paths.root.split(/[\\/]/).at(-1),
	]);
});
...
test("content and build changes use separate caches, regardless of file order", async (context) => {
	...
	const first = await resolveNornDocumentation(input);
	assert.equal(
		hashDocumentationBundle(bundle),
		hashDocumentationBundle({ ...bundle, files: [...bundle.files].reverse() }),
	);
	const changedBundle = {
		...bundle,
	...
	const changed = await resolveNornDocumentation({
		...input,
		source: { kind: "embedded", bundle: changedBundle },
	});
	const newBuild = await resolveNornDocumentation({
		...input,
		build: { ...build, commit: "b".repeat(40) },
	});
	assert.equal(
		new Set([first.paths.root, changed.paths.root, newBuild.paths.root]).size,
		3,
	);
...
for (const mutation of ["modified", "missing", "extra", "directory"]) {
	test(`a ${mutation} cache fails closed rather than advertising or overwriting altered evidence`, async (context) => {
		...
		if (mutation === "extra")
			await writeFile(join(result.paths.root, "extra.md"), "extra");
		if (mutation === "directory") await mkdir(join(result.paths.root, "extra"));
		await assert.rejects(
			resolveNornDocumentation(input),
			/cache is incomplete or modified/,
		);
		if (mutation === "modified")
			assert.equal(await readFile(result.paths.index, "utf8"), "changed");
		...
		assert.equal(
			await readFile(recovered.paths.index, "utf8"),
...
for (const target of ["file", "directory", "entry", "cacheRoot"]) {
	test(
		`a symlink at ${target} is rejected without changing its target`,
		...
		async (context) => {
			...
			await symlink(
				target === "file" ? join(outside, "sentinel") : outside,
				path,
			);
			await assert.rejects(
				resolveNornDocumentation(input),
				/cache .*modified|cache root must be a real directory/,
			);
			assert.equal(
...
test("byte verification does not conflate invalid UTF-8 with replacement characters", async (context) => {
	...
	const replacementBundle = {
		...bundle,
		files: bundle.files.map((file) =>
			file.path === "README.md" ? { ...file, content: "\ufffd" } : file,
		),
	};
	const replacementInput = {
		...input,
		source: { kind: "embedded" as const, bundle: replacementBundle },
	};
	...
	test(`reject unsafe or nonportable bundle path ${path}`, () => {
		assert.throws(
			() =>
				validateDocumentationBundle({
					...bundle,
					files: [...bundle.files, { path, content: "bad" }],
				}),
			/Invalid documentation asset path/,
		);
	});
}
...
test("reject duplicate/case-colliding files, file-directory collisions and missing required assets", () => {
	...
	assert.throws(
		() => validateDocumentationBundle({ ...bundle, files: [] }),
		/Missing documentation asset/,
	);
});
...
test("cache roots respect explicit override and native defaults", () => {
	assert.equal(
		...
		join("/home/test", "Library/Caches/norn/docs"),
	);
	assert.equal(
		resolveDocumentationCacheRoot({
			platform: "linux",
	...
	assert.equal(
		...
		join("/home/test", ".cache/norn/docs"),
	);
	assert.equal(
		resolveDocumentationCacheRoot({
			platform: "win32",
		...
		join("/local", "norn/docs"),
	);
	assert.equal(
		resolveDocumentationCacheRoot({
			platform: "darwin",
			home: "/home/test",
			environment: { NORN_DOCS_CACHE_DIR: "/override" },
		}),
		resolve("/override"),
	);
});
...
test("asset generation is deterministic, excludes itself, and includes every local Markdown link target", async (context) => {
	...
	assert.ok(
		assets.files.some((file) => file.path === "packages/sdk/src/api.ts"),
	);
	assert.ok(assets.files.some((file) => file.path === "docs/providers.md"));
	assert.ok(
		assets.files.some(
			(file) => file.path === "examples/minimal-workflow/.gitignore",
		),
	);
	assert.ok(
		!assets.files.some(
			(file) =>
				file.path.endsWith("documentation-assets.generated.ts") ||
				file.path.includes("/.norn/"),
		),
	);
	...
	for (const file of assets.files.filter((file) => file.path.endsWith(".md"))) {
		for (const match of file.content.matchAll(/\]\(([^)]+)\)/g)) {
			...
			assert.ok(
				paths.has(target) ||
					[...paths].some((path) =>
						path.startsWith(target.replace(/\/$/, "") + "/"),
					),
				`${file.path}: ${link}`,
			);
		}
	...
	const first = await generateDocumentationAssets({
		packageRoot,
		outputPath,
		assetRoot: join(root, "assets"),
	});
	const content = await readFile(outputPath, "utf8");
	const second = await generateDocumentationAssets({
		packageRoot,
		outputPath,
		assetRoot: join(root, "assets"),
	});
...
test("asset collection excludes host integrations and runtime/dependency files and refuses symlinked assets", async (context) => {
	...
	for (const file of [
		...bundle.files,
		{ path: "packages/cli/package.json", content: '{"version":"0.1.0"}' },
		{ path: "packages/sdk/src/api.ts", content: "export {};" },
		{ path: "packages/cli/src/cli.ts", content: "export {};" },
		{ path: "packages/core/src/agent-protocol.ts", content: "export {};" },
		{ path: "packages/pi-norn/src/index.ts", content: "export {};" },
		{ path: ".cursor-plugin/plugin.json", content: '{"name":"norn"}' },
		{ path: "tests/workflow-ref.test.ts", content: "" },
		{ path: "docs/providers.md", content: "# Provider setup\n" },
		{ path: "examples/.norn/runs/private.json", content: "private run" },
		{
			path: "examples/node_modules/dependency/index.ts",
			content: "dependency",
		},
		...
		await mkdir(join(root, ...file.path.split("/").slice(0, -1)), {
			recursive: true,
		});
	...
	assert.ok(
		!collected.files.some(
			(file) =>
				file.path.startsWith("packages/pi-norn/") ||
				file.path.startsWith(".cursor-plugin/"),
		),
	);
	assert.ok(
		!collected.files.some((file) =>
			/private run|dependency|credential|environment|key/.test(file.content),
		),
	);
	if (process.platform !== "win32") {
		await symlink(join(root, "README.md"), join(root, "docs/symlink.md"));
		await assert.rejects(
			collectDocumentationBundle({ packageRoot: root }),
