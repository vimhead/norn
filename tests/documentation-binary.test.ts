...
import {
	...
	generateDocumentationAssets,
} from "../scripts/generate-documentation-assets.ts";
...
test(
	"compiled binary resolves complete offline docs without source and runs an extracted example",
	{ timeout: 180_000 },
	async (context) => {
		...
		for (const path of [
			...
			"README.md",
			"package.json",
			"tests/workflow-ref.test.ts",
		]) {
			await mkdir(dirname(join(buildRoot, path)), { recursive: true });
			await cp(join(packageRoot, path), join(buildRoot, path), {
				recursive: true,
				filter: (source) =>
					!source.endsWith("documentation-assets.generated.ts"),
			});
		}
		await symlink(
		...
		await writeFile(
			join(buildRoot, "packages/cli/src/generated-build-info.ts"),
			`export const NORN_GENERATED_BUILD_INFO = ${JSON.stringify({ kind: "github-release-binary", version, commit, repository: "vimhead/norn", releaseTag: "tip", assetName: "norn-test", checksumAssetName: "norn-test.sha256" })} as const;\n`,
		);
		...
		await generateDocumentationAssets({
			packageRoot: buildRoot,
			outputPath: join(
				buildRoot,
				"packages/cli/src/bun/documentation-assets.generated.ts",
			),
			assetRoot: join(buildRoot, "packages/cli/assets"),
		});
		const expectedBundle = await collectDocumentationBundle({
			packageRoot: buildRoot,
		});
		const binary = join(
			detachedRoot,
			process.platform === "win32" ? "norn.exe" : "norn",
		);
		await execute(
			bun,
			[
				"build",
				"--compile",
				join(buildRoot, "packages/cli/src/bun/cli.ts"),
				"--outfile",
				binary,
			],
			{ cwd: buildRoot, timeout: 120_000, maxBuffer: 1024 * 1024 },
		);
		...
		const environment = {
			...
			NORN_PI_CACHE_DIR: join(root, "pi-assets"),
			NORN_DOCS_CACHE_DIR: cacheRoot,
			PATH: "",
			NODE_PATH: "",
		};
		const invoke = async (
			...
			const execution = execute(binary, args, {
				cwd,
				env: environment,
				timeout: 30_000,
		...
		assert.equal(
			(await invoke(["commands", "inspect", "docs.inspect"])).command.id,
			"docs.inspect",
		);
		assert.equal(
		...
		assert.equal(
			JSON.parse(
				(
					await execute(invocation[0], [...invocation.slice(1), "version"], {
						cwd: detachedRoot,
						env: environment,
						timeout: 30_000,
					})
		...
		const responses = await Promise.all(
			Array.from({ length: 4 }, () => invoke(["docs", "inspect"])),
		);
		const documentation = responses[0].documentation;
		for (const response of responses)
			assert.deepEqual(response.documentation, documentation);
		...
		for (const file of expectedBundle.files) {
			assert.deepEqual(
				await readFile(join(documentation.paths.root, file.path)),
		...
		await cp(
			join(documentation.paths.examples, "minimal-workflow"),
			projectRoot,
			{ recursive: true },
		);
		assert.equal(
		...
		const launch = await new Promise<{ run: NornRunInfo }>(
			(resolve, reject) => {
				const child = execFile(
					binary,
					["runs", "start", "greet"],
					{ cwd: projectRoot, env: environment, timeout: 30_000 },
					(error, stdout) => {
		...
		const finished = (
			await invoke(["runs", "wait", launch.run.id], projectRoot)
		).run;
		assert.equal(finished.status, "completed", JSON.stringify(finished));
		assert.equal(
			await readFile(join(finished.paths.workspace, "greeting.txt"), "utf8"),
			"Hello, Offline!\n",
		);
		const continuationRoot = join(root, "continuation-copy");
		await cp(
			join(documentation.paths.examples, "caller-selected-continuation"),
			continuationRoot,
			{ recursive: true },
		);
		const continuationInput = JSON.parse(
			await readFile(join(continuationRoot, "input.json"), "utf8"),
		);
		const continuationLaunch = await invoke(
			["runs", "start", "greetingProducer.write"],
			continuationRoot,
			continuationInput,
		);
		const delivered = (
			await invoke(
				["runs", "wait", continuationLaunch.run.id],
				continuationRoot,
			)
		).run;
		...
		assert.deepEqual(
			JSON.parse(
				await readFile(
					join(delivered.paths.workspace, "delivery.json"),
					"utf8",
				),
			),
			{
		...
		await cp(
			join(documentation.paths.examples, "shared-state"),
			join(projectRoot, "shared-state"),
			{ recursive: true },
		);
		await cp(
			join(
			...
			join(projectRoot, "work-queue.ts"),
		);
		await writeFile(
			join(projectRoot, "native.ts"),
			`
...
`,
		...
		await writeFile(
			join(projectRoot, "norn.project.json"),
			JSON.stringify({ workflows: ["./plugin.ts", "./native.ts"] }),
		);
		const nestedDirectory = join(projectRoot, "nested");
		await mkdir(nestedDirectory);
		const nativeLaunch = await invoke(
			["runs", "start", "native.check"],
			nestedDirectory,
			{
				args: {
					next: {
						workflow: "native.finish",
						forwardArgs: { origin: "queued" },
					},
				},
			},
		);
		const nativeResult = (
			await invoke(["runs", "wait", nativeLaunch.run.id], projectRoot)
		).run;
		assert.equal(
			nativeResult.status,
			"completed",
		...
		assert.deepEqual(nativeResult.outcome?.metadata?.data, {
			count: 42,
			origin: "queued",
			paths: {
				project: await realpath(projectRoot),
				workspace: join(nativeResult.path, "current/workspace"),
			},
		...
		assert.equal(
			(await invoke(["docs", "inspect"])).documentation.paths.root,
			documentation.paths.root,
		);

		const invokePi = (args: string[]) => {
			const execution = execute(binary, ["pi", ...args], {
				cwd: detachedRoot,
				env: environment,
				timeout: 30_000,
				maxBuffer: 2 * 1024 * 1024,
		...
		assert.equal(
			(await invokePi(["--version"])).stdout.trim(),
			piManifest.version,
		);
		assert.match(
			(await invokePi(["--help"])).stdout,
		...
		const oauthLogin = await invokePi([
			"--extension",
			oauthProbe,
			"--list-models",
			"openai-codex",
		...
		assert.equal(
			await realpath(join(agentDir, settings.packages[0])),
		...
		assert.match(
			(await invokePi(["--list-models", "norn-offline"])).stdout,
			/fixture/,
		);
		...
		assert.equal(
			JSON.parse(
				await readFile(join(response.packageRoot, "package.json"), "utf8"),
			).version,
			piManifest.version,
		);
		...
		await cp(
			join(packageRoot, "tests/fixtures/pi-worker-project"),
			workerProject,
			{ recursive: true },
		);
		const { run: workerRun } = await invoke(
			["runs", "start", "provider.check"],
			workerProject,
		);
		const completedWorker = (
			await invoke(["runs", "wait", workerRun.id], workerProject)
		).run;
		assert.equal(
			completedWorker.status,
			"completed",
			JSON.stringify(completedWorker),
		);
		assert.deepEqual(
			JSON.parse(
				await readFile(
					join(completedWorker.paths.workspace, "result.json"),
					"utf8",
				),
			),
			{ ok: true },
		...
		await assert.rejects(access(join(piAgentDir, "settings.json")), {
			code: "ENOENT",
		});
		...
		await assert.rejects(invokePi(["--version"]), (error) => {
			assert.match(
				...
				/Bundled Pi assets are incomplete or modified/,
			);
			return true;
		});
	},
