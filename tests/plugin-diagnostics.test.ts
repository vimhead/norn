...
type DiscoveryOutput = NornProjectLoadStatus &
	Partial<
		NornProjectInspection & NornWorkflowCatalogInfo & NornWorkflowInspection
	>;
type RunOutput = { run: NornRunInfo };
type ProjectErrorOutput = {
	error: Pick<
		NornProjectLoadError,
		"code" | "message" | "isComplete" | "diagnostics"
	>;
};
type CliResult<Output> = {
...
function createPluginSource({
	id,
	workflows = '{ step: { entrypoint: { instructions: "Use to complete the fixture step." }, args: Type.Object({}) } }',
	implementation = "{ workflows: { step: { execute: run => run.complete() } } }",
...
async function createFixture(
	...
	const cwd = await realpath(
		await mkdtemp(join(tmpdir(), "norn-plugin-diagnostics-")),
	);
	...
	await writeFile(
		join(cwd, "norn.project.json"),
		JSON.stringify({
			version: 1,
			workflows: Object.keys(files).filter((name) => name.endsWith(".ts")),
			config,
			includes,
		}),
	);
...
async function executeCli<Output>({
	cwd,
	args,
	input,
}: {
	...
	return new Promise((resolve, reject) => {
		const child = execFile(
			process.execPath,
			[cliPath, ...args],
			{ cwd, timeout: 20000, maxBuffer: 2 * 1024 * 1024 },
			(error, stdout, stderr) => {
				try {
					resolve({
					...
					reject(error ?? new Error(`Invalid CLI JSON: ${stdout}\n${stderr}`));
				}
			},
		);
...
function assertInvalidProject(
	result: CliResult<ProjectErrorOutput>,
...
test("discovery collects import, export, config and definition failures without losing valid siblings", async (context) => {
	const cwd = await createFixture(context, {
		files: {
			...
			"config.ts": createPluginSource({
				id: "config",
				configSchema: "Type.Object({ port: Type.Number() })",
			}),
			"callback.ts": 'throw new Error("definition setup failed");',
			"unguided.ts": createPluginSource({
				id: "unguided",
				workflows: "{ step: { entrypoint: {}, args: Type.Object({}) } }",
			}),
	...
	assert.deepEqual(
		result.workflows.map((workflow) => workflow.id),
		["good.step"],
	);
	assert.deepEqual(
		result.project.configurations
			.filter((owner) => owner.scopeId)
			.map((owner) => owner.key),
		["good"],
	);
	assert.equal(result.diagnostics.length, 5);
	const byFile = new Map(
		result.diagnostics.map((diagnostic) => [diagnostic.modulePath, diagnostic]),
	);
	for (const [file, stage] of [
		["broken.ts", "import"],
		["export.ts", "declaration"],
		["config.ts", "config"],
		["callback.ts", "import"],
		["unguided.ts", "import"],
	]) {
	...
	assert.match(
		byFile.get(join(cwd, "unguided.ts"))?.message ?? "",
		/instructions/,
	);
	assert.deepEqual(
		byFile
			.get(join(cwd, "config.ts"))
			?.issues.map((issue) => issue.instancePath),
		["/port"],
	);
	...
	await assert.rejects(
		loadNornProject(cwd),
		(error) =>
			error instanceof NornProjectLoadError && error.diagnostics.length === 5,
	);
});
...
test("a module with an invalid definition is excluded atomically", async (context) => {
	const cwd = await createFixture(context, {
		files: {
			"partial.ts": createPluginSource({
				id: "partial",
				workflows: `{
			valid: { entrypoint: { instructions: "Use to complete the valid step." }, args: Type.Object({}) },
			unguided: { entrypoint: {}, args: Type.Object({}) },
			missing: { entrypoint: { instructions: "Use to exercise missing implementation detection." }, args: Type.Object({}) },
			gate: { entrypoint: { instructions: "Use to exercise gate field validation." }, args: Type.Object({}), gate: { enabled: true, fields: ["unknown"] } }
		}`,
				implementation:
					"{ workflows: { valid: { execute: run => run.complete() }, unguided: { execute: run => run.complete() }, gate: { execute: run => run.complete() } } }",
			}),
			"good.ts": createPluginSource({ id: "good" }),
		},
	});
	...
	const missing = await inspectNornWorkflow({
		cwd,
		workflowId: "partial.valid",
	});
...
test("duplicate workflow and scope IDs exclude every conflicting source, including included configs", async (context) => {
	...
	await writeFile(
		join(cwd, "shared/plugin.ts"),
		createPluginSource({ id: "duplicate" }),
	);
	const result = await discoverNornProject(cwd);
	assert.equal(result.isComplete, false);
	assert.deepEqual(
	...
	const duplicates = result.diagnostics.filter(
		(diagnostic) => diagnostic.stage === "duplicate",
	);
	assert.ok(
		duplicates.every(
			(diagnostic) => diagnostic.workflowId === "duplicate.step",
		),
	);
	assert.deepEqual(
		duplicates.map((diagnostic) => diagnostic.configPath),
		[join(cwd, "norn.project.json"), join(cwd, "shared/norn.json")],
	);
	assert.deepEqual(
		duplicates.map((diagnostic) => diagnostic.modulePath),
		[join(cwd, "local.ts"), join(cwd, "shared/plugin.ts")],
	);
});
...
test("workflow ID conflicts between distinct plugins do not select an arbitrary winner", async (context) => {
	const cwd = await createFixture(context, {
		files: {
			"first.ts":
				createPluginSource({ id: "first" }) +
				'\nsteps.step.id = "shared.step";',
			"second.ts":
				createPluginSource({ id: "second" }) +
				'\nsteps.step.id = "shared.step";',
			"good.ts": createPluginSource({ id: "good" }),
		},
	});
	...
	assert.ok(
		result.diagnostics.every(
			(diagnostic) =>
				diagnostic.stage === "duplicate" &&
				diagnostic.workflowId === "shared.step",
		),
	);
	await assert.rejects(
		loadNornProject(cwd),
		/Workflow already registered: shared.step/,
	);
});

test("fresh discovery becomes complete after repairing the same source file", async (context) => {
	const cwd = await createFixture(context, {
		files: { "draft.ts": 'throw new Error("unfinished");' },
	});
	...
	assert.deepEqual(
		repaired.workflows.map((workflow) => workflow.id),
		["draft.step"],
	);
...
test(
	"CLI discovery reports incomplete status and schemas while execution fails before creating any run",
	{ timeout: 20000 },
	async (context) => {
		const cwd = await createFixture(context, {
			files: {
				"good.ts": createPluginSource({
					id: "good",
					workflows:
						'{ step: { entrypoint: { instructions: "Use to complete the fixture step." }, args: Type.Object({}) }, internal: { entrypoint: false, args: Type.Object({}) } }',
					implementation:
						"{ workflows: { step: { execute: run => run.complete() }, internal: { execute: run => run.complete() } } }",
				}),
				"broken.ts": 'throw new Error("broken sibling");',
				"another.ts": "export default false;",
			},
		});
		for (const args of [
			["project", "inspect"],
			["workflows", "list"],
			["workflows", "list", "--all"],
			["workflows", "inspect", "good.step"],
			["workflows", "inspect", "broken.step"],
		]) {
			const { exitCode, result } = await executeCli<DiscoveryOutput>({
				cwd,
				args,
			});
			...
			if (args[0] === "project") {
				...
				assert.equal(
					result.workflows.find((workflow) => workflow.id === "good.step")
						?.instructions,
					"Use to complete the fixture step.",
				);
			} else if (args[2] === "good.step") {
				assert.ok(result.workflow);
				assert.equal(result.workflow.argsSchema.type, "object");
				assert.equal(
					result.workflow.instructions,
					"Use to complete the fixture step.",
				);
			} else assert.equal(result.workflow, null);
		}
		assertInvalidProject(
			await executeCli<ProjectErrorOutput>({
				cwd,
				args: ["runs", "start", "good.step"],
				input: { args: {} },
			}),
			2,
		);
		await assert.rejects(stat(join(cwd, ".norn/runs")), { code: "ENOENT" });
		const conflictingFlags = await executeCli<{ error: { message: string } }>({
			cwd,
			args: ["workflows", "list", "--all", "--entrypoints"],
		});
...
test(
	"strict resume preserves the existing interruption and does not queue a request or acquire a lease",
	{ timeout: 20000 },
	async (context) => {
		const cwd = await createFixture(context, {
			files: {
				"gate.ts": createPluginSource({
					id: "gate",
					workflows:
						'{ step: { entrypoint: { instructions: "Use to decide whether to proceed." }, args: Type.Object({ approved: Type.Boolean() }), gate: { enabled: true, fields: ["approved"] } } }',
				}),
			},
		});
		const started = await executeCli<RunOutput>({
			cwd,
			args: ["runs", "start", "gate.step"],
			input: { args: { approved: false } },
		});
		assert.equal(started.exitCode, 0, JSON.stringify(started.result));
		const { run } = (
			await executeCli<RunOutput>({
				cwd,
				args: ["runs", "wait", started.result.run.id],
			})
		).result;
		...
		const resumed = await executeCli<ProjectErrorOutput>({
			cwd,
			args: ["runs", "resume", run.id],
			input: { args: { approved: true } },
		});
		assertInvalidProject(resumed, 2);
		const inspected = (
			await executeCli<RunOutput>({ cwd, args: ["runs", "inspect", run.id] })
		).result.run;
		...
		await writeFile(
			join(cwd, "broken.ts"),
			createPluginSource({ id: "repaired" }),
		);
		await writeFile(
			join(cwd, "another.ts"),
			createPluginSource({ id: "another" }),
		);
		assert.equal(
			(
				await executeCli({
		...
		assert.equal(
			(await executeCli<RunOutput>({ cwd, args: ["runs", "wait", run.id] }))
				.result.run.status,
			"completed",
		);
	},
...
test(
	"client discovery preserves status and diagnostics, and execution errors retain their structured details",
	{ timeout: 20000 },
	async (context) => {
		const cwd = await createFixture(context, {
			files: {
		...
		for (const result of [
			await client.project.inspect(),
			await client.workflows.list(),
			await client.workflows.inspect("good.step"),
		]) {
			assert.equal(result.isComplete, false);
			...
			if ("workflows" in result)
				assert.equal(
					result.workflows[0].instructions,
					"Use to complete the fixture step.",
				);
		}
		await assert.rejects(
			client.runs.start({ workflowId: "good.step", args: {} }),
			(error) =>
				error instanceof NornProjectLoadError &&
				error.diagnostics[0].stage === "import",
		);
		await assert.rejects(
			client.workflows.entries(),
			(error) =>
				error instanceof NornProjectLoadError && error.diagnostics.length === 1,
		);
		await writeFile(
			join(cwd, "broken.ts"),
			createPluginSource({ id: "repaired" }),
		...
		assert.equal(repaired.isComplete, true);
		assert.deepEqual(repaired.workflows.map((workflow) => workflow.id).sort(), [
			"good.step",
			"repaired.step",
		]);
...
test("an empty project is complete and an unknown workflow is an error only after complete loading", async (context) => {
	...
	const listed = await executeCli<NornWorkflowCatalogInfo>({
		cwd,
		args: ["workflows", "list"],
	});
	...
	await assert.rejects(
		inspectNornWorkflow({ cwd, workflowId: "unknown.step" }),
		/Unknown workflow/,
	);
});

test(
	"CLI and client inspection advertise contribution schemas without losing forwarded context during execution",
	{ timeout: 20000 },
	async (context) => {
		const source =
			...
			createPluginSource({
				id: "handoff",
				workflows: `{
			caller: { entrypoint: { instructions: "Use to collect records for the supplied task." }, args: Type.Object({ taskId: Type.String(), context: Type.Record(Type.String(), Type.Unknown()) }) },
			collect: { entrypoint: { instructions: "Use to collect records and forward them to the supplied continuation." }, args: Type.Object({ query: Type.String(), next: workflowRefSchema({ args: Type.Object({ records: Type.String() }) }) }) },
			finish: { entrypoint: false, args: Type.Object({ taskId: Type.String(), context: Type.Record(Type.String(), Type.Unknown()), records: Type.String() }) }
		}`,
		...
		const cwd = await createFixture(context, {
			files: { "handoff.ts": source },
		});
		const inspected = await executeCli<NornWorkflowInspection>({
			cwd,
			args: ["workflows", "inspect", "handoff.collect"],
		});
		...
		expect(schema).toHaveProperty(
			`${contributionPath}.properties.records.type`,
			"string",
		);
		expect(schema).toHaveProperty(
			"properties.next.anyOf.1.properties.forwardArgs.type",
			"object",
		);
		expect(schema).toHaveProperty(
		...
		expect(schema).toHaveProperty("properties.next.anyOf.1.required", [
			"workflow",
			"forwardArgs",
		]);
		const client = createNornClient({ spawnCwd: cwd, executablePath: cliPath });
		assert.deepEqual(
			await client.workflows.inspect("handoff.collect"),
			inspected.result,
		);
		const args = {
			taskId: "task-42",
			context: {
				labels: ["one", "two"],
				nested: { enabled: false, absent: null },
			},
		};
		const started = await executeCli<RunOutput>({
			cwd,
			args: ["runs", "start", "handoff.caller"],
			input: { args },
		});
		assert.equal(started.exitCode, 0, JSON.stringify(started.result));
		const completed = (
		...
		assert.deepEqual(
			JSON.parse(
				await readFile(join(completed.paths.workspace, "records.json"), "utf8"),
			),
			["recent incidents"],
		);
	},
...
test("unrepresentable contributions fail inspection as schema diagnostics rather than plugin import errors", async (context) => {
		...
		files: {
			"custom.ts":
				...
				createPluginSource({
					id: "custom",
					workflows:
						'{ step: { entrypoint: { instructions: "Use to inspect custom continuation parameters." }, args: Type.Object({ next: workflowRefSchema({ args: Type.BigInt() }) }) } }',
				}),
			"good.ts": createPluginSource({ id: "good" }),
		},
	});
	...
	const inspected = await inspectNornWorkflow({
		cwd,
		workflowId: "custom.step",
	});
	assert.equal(inspected.workflow, null);
	assert.equal(inspected.isComplete, false);
	assert.deepEqual(
		inspected.diagnostics.map((diagnostic) => diagnostic.stage),
		["schema"],
	);
	assert.equal(inspected.diagnostics[0].workflowId, "custom.step");
	assert.equal(
		(await inspectNornWorkflow({ cwd, workflowId: "good.step" })).isComplete,
		true,
	);
});
...
test("inspection reports an unrepresentable args schema without hiding sibling diagnostics", async (context) => {
	const cwd = await createFixture(context, {
		files: {
			"custom.ts": createPluginSource({
				id: "custom",
				workflows:
					'{ step: { entrypoint: { instructions: "Use to inspect custom parameters." }, args: Type.BigInt() } }',
			}),
			"broken.ts": 'throw new Error("unfinished");',
		},
	});
