...
async function exercisePublication(input: {
	...
	const packageNames = [
		"@vimhead.dev/norn",
		"@vimhead.dev/norn-cli",
		"@vimhead.dev/pi-norn",
	];
	const packages = PUBLISHED_WORKSPACES.map((workspace, index) => ({
	...
	const files = new Map<string, string | Buffer>([
		[
			join(workspaceRoot, "dist/npm/release.json"),
			JSON.stringify({ packages }),
		],
	]);
	for (const [index, workspace] of PUBLISHED_WORKSPACES.entries()) {
		files.set(
			join(workspaceRoot, "packages", workspace, "package.json"),
			JSON.stringify({ name: packages[index].name, version }),
		);
		files.set(
			join(workspaceRoot, "dist/npm", packages[index].filename),
			tarball,
		);
	}
	...
	vi.doMock("node:fs/promises", async (importOriginal) => ({
		...
		readFile: async (path: string) => {
			...
			if (contents === undefined)
				throw new Error(`Unexpected publication fixture path: ${path}`);
			return contents;
		},
	}));
	...
	process.argv = [
		process.execPath,
		join(workspaceRoot, "scripts/publish-packages.ts"),
		"--interactive",
	];
	try {
