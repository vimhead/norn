...
export default async function checkCodexOAuthLogin(): Promise<void> {
	...
	await assert.rejects(oauth.login({
		signal: new AbortController().signal,
		async prompt(request) {
			assert.ok(request.type === "select");
			assert.deepEqual(request.options.map(option => option.id), ["browser", "device_code"]);
			throw cancelled;
		},
		notify() { assert.fail("OAuth probe must stop before authorization"); },
	}), error => {
