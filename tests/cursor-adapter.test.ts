...
async function runHook(input: { readonly cwd: string; readonly env?: NodeJS.ProcessEnv }): Promise<HookResult> {
	const { stdout, stderr } = await runExecutable(process.execPath, [hookScript], {
...
test("Cursor session hook injects the runtime-selected Norn introduction", async context => {
	...
	await writeExecutable(join(fixture.root, "norn"), `
...
`);
	const result = await runHook({ cwd: fixture.cwd, env: { PATH: `${fixture.root}${delimiter}${process.env.PATH ?? ""}`, NORN_EXECUTABLE: "" } });
...
test("Cursor hook passes explicit executable paths as executable names", async context => {
	...
	await writeExecutable(executable, 'process.stdout.write(JSON.stringify({ intro: "Custom runtime introduction" }));');
	const result = await runHook({ cwd: fixture.cwd, env: { NORN_EXECUTABLE: executable } });
	assert.equal(result.stderr, "");
	assert.deepEqual(JSON.parse(result.stdout), { additional_context: "<norn-docs-intro>\nCustom runtime introduction\n</norn-docs-intro>" });
});

const failures: [string, string][] = [
	["nonzero exit", 'process.stderr.write("secret diagnostic"); process.exit(1);'],
	...
	["oversized intro", `process.stdout.write(JSON.stringify({ intro: "a".repeat(16_385) }));`],
];
