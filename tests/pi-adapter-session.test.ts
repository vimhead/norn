...
function captureModelRequests(
	...
	session.agent.streamFunction = (_model, context) => {
		...
		if (resourceCall) {
			assert.ok(
				tools.some((tool) => tool.name === resourceCall.name),
				`Attached tool is active: ${resourceCall.name}`,
			);
			message.content = [
				{ type: "toolCall", id: `resource-${nextCall}`, ...resourceCall },
			];
			message.stopReason = "toolUse";
		}
		stream.push({
...
test(
	"Pi loads runtime-selected context before the first prompt and refreshes it on reload",
	{ skip: process.platform === "win32", timeout: 60_000 },
	async (context) => {
		const fixture = await createFixture(context);
		const runtimeRoot = join(fixture.root, "other installation");
		await mkdir(runtimeRoot);
		for (const path of ["dist", "bin", "assets", "package.json"])
			await cp(
				join(packageRoot, "packages/cli", path),
				join(runtimeRoot, path),
				{ recursive: true },
			);
		await symlink(
		...
		await writeFile(
			runtimeManifestPath,
			JSON.stringify({ ...runtimeManifest, version: manifest.version }),
		);
		...
		const directIntro = JSON.parse(
			execFileSync(join(runtimeRoot, "bin/norn.mjs"), ["docs", "intro"], {
				cwd: fixture.cwd,
				encoding: "utf8",
			}),
		);
		...
		const settingsManager = SettingsManager.inMemory({
			packages: [join(packageRoot, "packages/pi-norn")],
		...
		const loader = new DefaultResourceLoader({
			...
			systemPromptOverride: (base) =>
				useCustomPrompt ? "CUSTOM SYSTEM PROMPT" : base,
			agentsFilesOverride: () => ({
				agentsFiles: [
					{ path: join(fixture.cwd, "AGENTS.md"), content: "PROJECT CONTEXT" },
				],
			}),
			extensionFactories: [
				{
					name: "other-prompt-extension",
					factory: (pi) => {
		...
		assert.ok(
			loader
				...
				.extensions.some((extension) =>
					extension.path.endsWith("pi-norn/dist/index.js"),
				),
		);
		...
		const invoke = <Output>(args: readonly string[]): Output =>
			JSON.parse(
				execFileSync(invocation[0], [...invocation.slice(1), ...args], {
					cwd: fixture.cwd,
					encoding: "utf8",
					timeout: 30_000,
				}),
		...
		assert.deepEqual(
			invoke<NornWorkflowCatalogInfo>(["workflows", "list"]).workflows,
			[],
		);
		const plugin = (
			await readFile(
				join(packageRoot, "examples/minimal-workflow/plugin.ts"),
				"utf8",
			)
		).replace('name: "greet"', 'name: "freshGreeting"');
		await writeFile(join(fixture.cwd, "plugin.ts"), plugin);
		await writeFile(
			join(fixture.cwd, "norn.project.json"),
			JSON.stringify({ version: 1, workflows: ["./plugin.ts"] }),
		);
		assert.deepEqual(
			invoke<NornWorkflowCatalogInfo>(["workflows", "list"]).workflows.map(
				(workflow) => workflow.id,
			),
			["freshGreeting"],
		);
		const inspection = invoke<NornWorkflowInspection>([
			"workflows",
			"inspect",
			"freshGreeting",
		]);
		...
		assert.ok(
			!lastRequest(captured).systemPrompt.includes('Version: "9.9.10"'),
		);
		const unrelatedSkillPath = join(
			fixture.agentDir,
			"skills/test-guidance/SKILL.md",
		);
		await mkdir(join(fixture.agentDir, "skills/test-guidance"), {
			recursive: true,
		});
		await writeFile(
			unrelatedSkillPath,
		...
		assert.equal(
			lastRequest(captured).systemPrompt.split("<norn-docs-intro>").length - 1,
			1,
		);
		...
		assert.ok(
			lastRequest(captured).systemPrompt.startsWith(
				"You are an expert coding assistant",
			),
		);
		assert.ok(lastRequest(captured).systemPrompt.includes("Pi documentation"));
		assert.ok(
			lastRequest(captured).systemPrompt.includes("<available_skills>"),
		);
		assert.equal(
			lastRequest(captured).systemPrompt.split("<norn-docs-intro>").length - 1,
			1,
		...
		assert.ok(
			!lastRequest(captured).systemPrompt.includes("<norn-docs-intro>"),
		);
		await session.reload();
		await session.prompt("Use the PATH runtime now");
		assert.ok(
			lastRequest(captured).systemPrompt.includes("PATH runtime introduction"),
		);
		assert.ok(
			!lastRequest(captured).systemPrompt.includes('Version: "9.9.10"'),
		);
		loader
			...
			.runtime.flagValues.set(
				"norn-executable",
				join(fixture.root, "missing-executable"),
			);
		await session.reload();
...
test(
	"a real native Norn worker excludes the adapter, including after reload with its response tool inactive",
	...
	async (context) => {
		...
		await writeExecutable(
			executable,
			`require("node:fs").writeFileSync(${JSON.stringify(calledPath)}, "called"); process.stdout.write(JSON.stringify({intro:"UNWANTED AUTHORING CONTEXT"}));`,
		);
		const previousPath = process.env.PATH;
		...
		assert.deepEqual(
			await worker.prompt({
				prompt: "Assess only this supplied source.",
				response: Type.Object({ ok: Type.Boolean() }),
				maxAttempts: 1,
			}),
			{ ok: true },
		);
		const decoded = await worker.prompt({
			prompt: "Record the next assessment.",
			response: Type.Decode(Type.Object({ ok: Type.Boolean() }), (result) =>
		...
		assert.ok(
			session
				.getAllTools()
				.some((tool) => tool.name === AGENT_RESPONSE_TOOL_NAME),
		);
		...
		await session.prompt(
			"Check the same context with the response tool inactive",
		);
...
test(
	"native custom state tools retain values across sessions and session cleanup covers startup failure",
	{ timeout: 30000 },
	async (context) => {
		const fixture = await createFixture(context);
		...
		const customTools = createStateTools({
			state,
		...
		assert.deepEqual(
			await worker.prompt({
				prompt: "Exercise attached state",
				response: Type.Object({ ok: Type.Boolean() }),
				maxAttempts: 1,
			}),
			{ ok: true },
		...
		const results = nativeSessions[0].messages.filter(
		...
		assert.deepEqual(
			new Set(lastRequest(captured).tools),
			new Set([AGENT_RESPONSE_TOOL_NAME, "norn_state_get"]),
		);
...
test(
	"plain custom tools follow Pi defaults, explicit selection, and loaded-extension collisions",
	{ timeout: 30000 },
	async (context) => {
		const fixture = await createFixture(context);
		...
		await writeFile(
			join(fixture.agentDir, "extensions", "extra.ts"),
			`export default pi => {
		...
		const readDraft: ToolDefinition = {
			name: "read_draft",
			label: "Read draft",
			description: "Read the workflow's draft.",
			...
			.mockImplementation(async function (this: AgentSession, ...args) {
				if (!nativeSessions.includes(this)) {
					...
					captureModelRequests(
						this,
						captured,
						shouldReadDraft ? [{ name: readDraft.name, arguments: {} }] : [],
					);
				}
				return originalPrompt.apply(this, args);
			});
		...
		for (const scenario of [
			...
			{
				tools: ["read", readDraft.name, "extra_tool"],
				expected: ["read", readDraft.name, "extra_tool"],
			},
		]) {
			...
			assert.deepEqual(
				new Set(lastRequest(captured).tools),
				new Set([...scenario.expected, AGENT_RESPONSE_TOOL_NAME]),
			);
		...
		await writeFile(
			join(fixture.agentDir, "settings.json"),
			JSON.stringify({ defaultTools: ["grep"] }),
		);
