...
import {
	...
	work,
	verify,
} from "../examples/coordinating-multiple-agents/plugin.ts";
...
async function createWorkflowFixture(
	...
	const firstRoundClaims = new Promise<void>((resolve) => {
	...
	const promptSpy = vi
		...
		.mockImplementation(async function (this: AgentSession, ...args) {
			if (!sessions.has(this)) {
				...
				this.agent.streamFunction = async (_model, request) => {
					...
					if (input.isReportOnly) {
						call = {
							...
							arguments: {
								...
								response: {
									status: "acknowledged",
									detail: "An unsupported success claim.",
								},
							},
						};
					} else if (phase === 0) {
						...
						const response = request.messages
							.filter(
								(message) =>
									...
									message.toolName === "queue_claim",
							)
							.at(-1);
						assert.ok(
							response && response.role === "toolResult" && !response.isError,
						);
						const text = response.content.find((part) => part.type === "text");
						assert.ok(text && text.type === "text");
						const { claim } = JSON.parse(text.text) as {
							claim: { id: string; token: string; text: string } | null;
						};
						if (claim === null) {
							call = {
								name: AGENT_RESPONSE_TOOL_NAME,
								arguments: {
									runId,
									label,
									response: { status: "idle", detail: "No note available." },
								},
							};
						} else {
							...
							if (label.startsWith("round-0-")) {
								if (
									claims.filter((claim) => claim.label.startsWith("round-0-"))
										.length === 2
								)
									releaseFirstRound();
							...
							call = {
								...
								arguments: {
									...
									token: claim.token,
									result: {
										summary: `Summary of ${claim.id}.`,
										quote: input.shouldInventQuote
											? "A quotation absent from every source."
											: claim.text,
									},
								},
							};
						...
						const response = request.messages
							.filter(
								(message) =>
									message.role === "toolResult" &&
									message.toolName === "queue_acknowledge",
							)
							.at(-1);
						assert.ok(
							response && response.role === "toolResult" && !response.isError,
						...
						call = {
							...
							arguments: {
								...
								response: {
									status: "acknowledged",
									detail: "Result saved through the queue tool.",
								},
							},
						};
					}
					...
					const message: AssistantMessage = {
						...
						model: model.id,
						content: [{ type: "toolCall", id: `offline-${phase}`, ...call }],
						stopReason: "toolUse",
						timestamp: Date.now(),
						usage: {
							input: 0,
			...
			activePrompts++;
			maximumConcurrentPrompts = Math.max(
	...
	await cp(
		new URL("../examples/coordinating-multiple-agents/", import.meta.url),
		root,
		{ recursive: true },
	);
	...
	return {
		...
		claims,
		activeTools,
		maximumConcurrentPrompts: () => maximumConcurrentPrompts,
		repairModel: () => {
...
async function inspectQueue(runRoot: string) {
	const workspace = (await getRunInfo(runRoot)).paths.workspace;
	const queue = await WorkQueue.open({
		path: join(workspace, "queue.sqlite"),
		create: false,
...
for (const noteCount of [3, 4]) {
	test(
		`the copied example coordinates Norn agent sessions and verifies ${noteCount} persisted results`,
		{ timeout: 30000 },
		async (context) => {
			const setup = await createWorkflowFixture(context, {
				failureLabel: undefined,
				shouldInventQuote: false,
			});
			const selectedNotes = notes.slice(0, noteCount);
			const result = await setup.engine.runWorkflow(
				start,
				{ notes: selectedNotes },
				undefined,
			);
			...
			assert.deepEqual(result.metadata?.data, {
				summariesPath: "summaries.json",
				processed: noteCount,
			});
			const runRoot = join(setup.root, ".norn/runs", result.id);
			const saved = JSON.parse(
				await readFile(
					join((await getRunInfo(runRoot)).paths.workspace, "summaries.json"),
					"utf8",
				),
			);
			assert.deepEqual(
				saved.map((item: { id: string }) => item.id),
				selectedNotes.map((note) => note.id),
			);
			assert.ok(
				saved.every(
					(item: { source: string; quote: string; deliveries: number }) =>
						item.source.includes(item.quote) && item.deliveries === 1,
				),
			);
			assert.equal(setup.maximumConcurrentPrompts(), 2);
			assert.equal(setup.sessions.size, 4);
			assert.equal(
				new Set(setup.claims.map((claim) => claim.id)).size,
				noteCount,
			);
			for (const tools of setup.activeTools)
				assert.deepEqual(
					new Set(tools),
					new Set([
...
test("agent success reports without persisted results cannot complete the example", async (context) => {
	...
	assert.equal(
		(await inspectQueue(join(setup.root, ".norn/runs", result.id)))
			.acknowledged,
		0,
	);
});
...
test(
	"acknowledgment is not semantic approval: the example rejects invented quotations",
	{ timeout: 30000 },
	async (context) => {
		const setup = await createWorkflowFixture(context, {
			failureLabel: undefined,
			shouldInventQuote: true,
		});
		await assert.rejects(
			setup.engine.runWorkflow(start, { notes }, { id: "invalid-quotes" }),
			/Unverified result or source quotation/,
		);
		const runRoot = join(setup.root, ".norn/runs/invalid-quotes");
		assert.equal((await getRunInfo(runRoot)).status, "failed");
		assert.equal((await inspectQueue(runRoot)).acknowledged, 4);
		await assert.rejects(
			readFile(
				join((await getRunInfo(runRoot)).paths.workspace, "summaries.json"),
			),
...
test(
	"native rollback and fresh Norn agents retain checkpointed results and retry only the interrupted round",
	{ timeout: 30000 },
	async (context) => {
		const setup = await createWorkflowFixture(context, {
			failureLabel: "round-1-worker-2",
			shouldInventQuote: false,
		});
		await assert.rejects(
			setup.engine.runWorkflow(start, { notes }, { id: "recover-queue" }),
			/Queue agents failed/,
		);
		const runRoot = join(setup.root, ".norn/runs/recover-queue");
		assert.equal((await getRunInfo(runRoot)).status, "failed");
		await cp(
			join(runRoot, "current"),
			join(setup.root, "preserved-failed-attempt"),
			{ recursive: true },
		);
		...
		assert.ok(
			setup.claims
				.slice(priorClaims)
				.every((claim) => !completedIds.includes(claim.id)),
		);
		assert.ok([...setup.sessions].every((session) => !session.isStreaming));
	},
);
